...
def get_source_root():
    ...
    for candidate in candidates:
        if os.path.exists(os.path.join(candidate, HTML_FILE)) and os.path.isdir(
            os.path.join(candidate, "automated_scripts")
        ):
            return candidate
    return script_dir
    ...
    def _handle_run(self):
        ...
        try:
            ...
            if (script_name.endswith('.py')):
                ...
                try:
                    if getattr(sys, "frozen", False):
                        rel_script = os.path.relpath(file_path, WORK_ROOT)
                        use_exe = sys.executable
                        launch_args = [str(use_exe), "--run-script", rel_script]
                        print(f"[-] Frozen self-launch: {rel_script} using {use_exe}")
                    else:
                    ...
                    if getattr(sys, "frozen", False):
                        rel_script = os.path.relpath(file_path, WORK_ROOT)
                        subprocess.Popen([str(sys.executable), "--run-script", rel_script], cwd=script_cwd)
                    else:
