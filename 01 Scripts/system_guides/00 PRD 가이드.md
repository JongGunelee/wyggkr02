...
**문서 생성일**: 2026-01-17  
**최종 업데이트**: 2026-09-18 (v39.3.0 PowerPoint & Excel Hyper Compressor Fusion & Stability Hardening Update)
**작성자**: Antigravity (AI Assistant)  
**관리 도구**: Python / win32com (COM Auto-Elevation) / Taskkill (Force Cleanup) / PyMuPDF / Tkinter / Pillow
**시스템 버전**: v39.3.0 Domain-Driven / Launcher v3.5.8 Stable
**설치된 스킬스 및 에이전트**:
...
### ❌ [실패 사례 3]: Tkinter 동적 레이아웃의 고정 높이 함정 및 정규식 불완전 매칭
- **발생 상황**: '지능형 파일 정리기' UI에서 규칙 추가 시 패널 높이가 자동 확장되지 않거나, 패턴 제안 기능이 닫는 괄호(')')를 누락하여 원본 파일명을 파손함.
- **실패 원인**: 
    1. 레이아웃: `grid_propagate(True)` 설정으로 인해 내부 위젯 크기 변화가 외부 프레임 설정을 무시하거나, 고정 픽셀(px) 기반 계산이 누락됨.
    2. 로직: `r'(\([^\)]+)'` 처럼 닫는 괄호를 포함하지 않는 불완전 정규식과 `suffix=')'` 하드코딩의 결합으로 비논리적 패턴 생성.
- **해결 패턴 (성공 조치)**:
    1. **Layout Hardening**: `grid_propagate(False)`와 `Text.bbox` 기반 픽셀 좌표 측정 로직을 도입하여 임계치(120px) 초과 시에만 스크롤바가 생성되는 '유동적 동적 레이아웃' 완성.
    2. **Logic Hardening**: 다중 괄호((), [], {}) 및 날짜/토큰을 모두 지원하는 완전 정규식으로 교체하고, 규칙 추가 시의 하드코딩된 suffix를 제거하여 데이터 정합성 100% 확보.
...
### 3.56 [File Management] 지능형 파일 고속 관리기 듀얼 모드(File/Folder) 및 백그라운드 점유 무력화 (v34.1.29)
- **요구사항**: 
    - 기존 파일명 단위의 스마트 관리를 넘어 **[폴더명(Directory)]** 확장 관리를 위한 듀얼 모드(File/Folder) UI 지원.
    - 파일 탐색기 및 사내 프로그램이 폴더의 경로를 물고 있어(Handle Hold) 발생하는 `PermissionError` 버그를 완전히 정복하고 조작 무결성 달성.
    - 대량/대용량 폴더(약 3GB) 복사 시 프로그램 멈춤(Freezing)으로 오인되지 않도록 사전에 시간 소요를 경고하는 지능적 알림 체계 구비.
- **핵심 구현 (Force Unlock & Atomic Processing)**:
    - **Dual-Mode UI Architecture**: 파일 및 폴더 라디오 옵션을 통해 UI 확장자 필터를 자동 비활성화하고 컬럼 헤더들을 동기화하여 직관성 극대화. 엔진부는 `scan_files` 단계에서 디렉토리 전용 필터링 분기.
    - **psutil 방탄 강제 점유 해제 (Force Unlock in While Loop)**: 이름 변경 등 조작 대상을 타 프로세스(`explorer.exe` 등)가 점유하고 있으면, PID와 이름을 모달창에 즉시 띄워 수동 승인을 받아 **강제 Kill 후 재시도(Retry)**하여 작업 프로세스가 중단되지 않는 방어 체계 적용.
    - **Pre-Validation 타임캡 예측**: 깊이 탐색(`get_dir_size`)으로 복사본 생성 전 용량을 사전 계산하고 3GB(1분 예상) 초과 시 안전 주의(`askyesno`) 경고 노출.
    - **지능형 숫자 정렬 (Smart Sorting)**: Treeview의 'NO' 열 클릭 시 문자열 순서(1, 10, 2)가 아닌 실제 숫자 크기 순서로 정렬되도록 리스트 변환 알고리즘 적용.
    - **무결성 추적 리포트 (CSV Upgrade)**: 원본 파일과 결과 파일의 크기를 1byte 단위까지 정교하게 비교('원본 크기', '결과 크기', '크기 차이')하여 CSV 리포트에 자동 기록.
- **수행 도구**: `intelligent_file_organizer.py` **[v34.1.30]**

### [v34.1.30] UI 경험 고도화: 개별 추가 기능 및 폴더 선택 UX 리팩토링 (2026-04-08)
- **버튼 명칭 변경**: `개별 파일 추가` -> **`개별 추가`** (파일/폴더 모드 통합 대응)
- **지능형 멀티 폴더 선택기 (FolderSelectorDialog) 고도화**:
    - **주소창(Address Bar) 및 이동 버튼**: 윈도우 탐색기 스타일의 경로 입력 및 즉시 이동 기능 탑재.
    - **최종 경로 자동 추적 및 확장**: 세션 내 마지막 사용 폴더를 기억하여 창이 열릴 때 해당 위치까지 자동으로 레이지 로딩 및 트리를 펼쳐주는 지능형 자동화 구현.
    - **UI 단순화 및 아이콘 적용**: 불필요한 경로 열을 제거하고 드라이브(🖴), 폴더(📁) 아이콘을 적용하여 직관성 극대화.
- **상단바 제어 버튼 집결 및 최적화**:
    - **버튼 이동 배치**: `미리보기`, `작업 중단(Stop)`, `[실행]` 버튼을 상단 '목록 초기화' 우측으로 이동하여 작업 동선을 상단으로 집중.
    - **기존 외형 완벽 복원**: 이동된 버튼들의 색상, 텍스트(이모지 포함), 폰트 크기를 기존 버전과 동일하게 복구하여 사용자 경험의 연속성 확보.
    - **가로 누적형 칩(Chip) UI 도입**: [패턴 제안], [적용 규칙 목록]을 세로 리스트박스에서 가로 칩 프레임으로 변경.
    - **입력창 높이 50% 축소**: 위젯 높이를 반감하여 세로 공간을 극적으로 절약하고 한 화면에 더 많은 규칙 정보를 제공.
    - **레이아웃 무결성 확보**: 버튼 이동 시 `path_entry`의 너비를 정밀 조정(80->65)하고 컴팩트 스타일을 적용하여 가로 폭 붕괴 방지.
    - **공간 효율성 극대화**: 규칙 패널 내 불필요한 중복 타이틀 제거를 통해 세로 공간을 확보하고 조작 편의성 증대.
    - **핵심 버튼 스타일 통일**: '규칙 추가' 버튼과 상단 '[실행]' 버튼의 스타일을 Blue 테마로 단일화하여 조작 일관성 확보.
    - **시각적 줄바꿈 기반 동적 확장**: 논리적 줄 수가 아닌 실제 화면에 표시되는 시각적 줄 수(Display Lines)를 정밀 감지하여 하단 객체들을 유동적으로 밀어내고 레이아웃을 즉시 갱신하는 무결성 구현.
    - **가이드 안내**: 상단 팁 메시지를 통해 사용자에게 조작 방식(클릭: 진입 / Ctrl+클릭: 선택)을 명확히 전달.

- **사용 Agent 및 Skills**:
    - **Agent**: `Antigravity` (Advanced Agentic AI Assistant)
    - **Primary Skills**:
        - `agentic-engineering`: 클린 레이어 아키텍처 준수 및 UI/컨트롤러 로직 분리 설계.
        - `systematic-debugging`: 경로 추적 및 자동 확장 로직의 예외 상황(권한 오류 등) 사전 차단.
        - `lint-and-validate`: 구문 무결성 전수 점검을 통한 런타임 안정성 확보 (`py_compile`).
        - `documentation`: 작업 이력의 정밀한 기록 및 PRD 가이드라인 형상 관리.
...
### 3.45 [B2B Mapping & Dashboard] B2B 발주내역 월말마감 스마트 매핑 솔루션 통합 및 Smart Action Queue 무결성 구현 (v3.0 / v35.4.19)
- **요구사항**:
    - `b2b_mapping_automation.py`를 `01 Scripts/automated_scripts/` 폴더에 통합 복사하고, 대시보드(`00 dashboard.html`)의 '📝 PR견적 구성 활용' 카테고리 바로 밑에 신규 카테고리 '📅 월말 마감 검토'(`closing`)를 신설하여 자동 등록 및 지속 확장성 확보.
    - 백그라운드 엑셀 파싱 중 사용자의 연속 버튼 클릭 시 발생하는 엑셀 창 닫힘 및 COM 충돌 현상을 원천 차단하는 무결성 리팩토링 구현.
- **핵심 개선 (Smart Action Queue & COM Isolation Architecture)**:
    - **COM 인스턴스 완전 격리 (DispatchEx)**: 백그라운드 소스/타겟 파일 사전 파싱 시 `DispatchEx`를 사용하여 사용자의 메인 엑셀 인스턴스와 바인딩되는 현상 차단.
    - **사용자 제어권 보장 (UserControl = True)**: 대화형 엑셀 범위 선택 창 열기 시 `excel.UserControl = True`를 주입하여 파이썬 COM 개체 연결 해제 시에도 엑셀이 꺼지지 않도록 유도.
    - **스마트 액션 큐 (Smart Action Queue)**: 파싱 진행 상태(`active_tasks`) 모니터링 및 버튼 텍스트 다이내믹 변경(예: "⏳ 분석 중 (자동 예약)"). 파싱 중 사용자 클릭 시 액션을 큐(`queued_action`)에 예약하고, 파싱 완료 이벤트(`root.after`)에서 0.3초 내에 자동 트리거.
    - **2단계 불일치 검수 및 서식 커밋**: 최종 검수 리포트에서 수동 불일치 선택 항목에 대해 값 입력은 스킵하고 타겟 계약명 셀(24열)을 붉은색(Red, ColorIndex 255)으로 시각적 강조 커밋 처리.
    - **Clean Architecture 3계층 준수 & 이식성 자산화**: Domain Engine, Presentation View, Application Controller 3계층 분리로 향후 웹 UI(Flask/FastAPI REST API)로의 이식 가이드를 `AI_CODING_GUIDELINES_2026.md`에 자산화 반영.
- **대상 파일**: 
    - `automated_scripts/b2b_mapping_automation.py` (**v3.0**)
    - `00 dashboard.html` (**v35.4.19**)
...
#### 4.1.2 [최적화] 파워포인트 전용 압축기 2세대 완전 융합 (v39.3.0)
- **요구사항**: 구형 1세대 도구의 단일 파일 선택 한계 및 백업 찌꺼기 잔존 결함을 극복하고, 폴더 재귀 탐색, 트리 보존/단일 저장 듀얼 모드, 자동 순번 접미사(`_1`, `_2`), 원자적 단일 교체 및 ZIP CRC/PowerPoint 열기 무결성 검증을 총망라한 2세대 엔진 구축.
- **구현**: `ppt_image_compression_batch.py` (구 `ppt_compressor_tool.py` 대체 융합 배포).
- **특이사항**: 슬라이드 XML의 `cx/cy` 좌표는 완전 보존하고 `a:srcRect` 크롭 영역을 영구 절삭하여 데이터 안전성과 압축률(평균 50~80% 감소) 동시 달성.
- **배포 위치**: `01 Scripts/automated_scripts/ppt_image_compression_batch.py`
...
#### 4.2.4 [네이밍] 지능형 파일 관리 및 패턴 기반 정규화 (v2.0 Ultimate)
- **요구사항**: 복잡한 파일명/폴더명 패턴을 지능적으로 감지하여 단축(Truncate), 치환, 정규화 수행 및 고충실도 UI 제공.
- **기능**:
    - **지능형 다중 패턴 엔진 (v34.1.41)**: 소괄호(), 대괄호[], 중괄호{} 및 날짜 패턴과 고빈도 키워드(Token)를 자동 분석하여 최적의 마커 제안.
    - **파일/폴더 모드별 정합성**: 파일 모드 시 확장자를 자동 제외하여 분석하고, 폴더 모드 시 전체 명칭을 분석하는 모드별 특화 로직.
    - **유동적 동적 레이아웃 (Flow Layout)**: 추가되는 규칙/패턴 수에 따라 패널 높이가 45px에서 120px까지 자동 확장되며, 초과 시 세로 스크롤바가 자동 생성되는 UX 탑재.
    - **무결성 정밀 진단**: 실행 전 중복 파일 및 기존 파일 충돌 여부를 시각화하고 실시간 미리보기 제공.
- **도구**: `intelligent_file_organizer.py`
- **상태**: UI/UX 및 추출 엔진 고도화 배포 완료. (2026-04-08)
...
### 3.68 [File Management] 지능형 파일 고속 관리기 디자인 고도화 및 패턴 엔진 지능화 (v34.1.41)
- **요구사항**: 사용자 피드백 기반 45px 슬림 UI 구축 및 다중 패턴 감지 엔진의 논리적 정합성 전수 점검.
- **핵심 기술**:
    - **Asymmetric Slim Layout**: [패턴 제안] 패널 45px, [규칙 목록] 패널 45px로 슬림화 및 비대칭/대칭형 유동적 전환.
    - **Intelligent Pattern Engine**: 소괄호, 대괄호, 중괄호 전수 추출 및 날짜/토큰 빈도 기반 제안 시스템 고도화.
    - **Dynamic Scroll Synchronization**: 가속 가변 높이 패널과 상위 캔버스 스크롤 영역의 실시간 동기화로 레이아웃 파손 방지.
    - **Zero-Failure Integrity Patch (v34.1.41-Final)**: 
        - [경로 정규화] `/`와 `\` 혼용을 `normpath`로 원천 통일하여 윈도우 환경 스캔 무결성 100% 확보.
        - [렌더링 확정] `root.update()` 및 `height` 강제 고정을 통해 '로그에는 존재하나 목록에는 부재'하는 현상 차단.
        - [교차 검증] 내부 데이터 개수와 Treeview 보유 아이템 수를 실시간 대조하는 자가 진단 로직 탑재.
- **수행 도구**: `intelligent_file_organizer.py` **[v34.1.41]**
- **상태**: 결함 및 가시성 패치 최종 완료 (2026-04-08)

**🎯 결론**: 현재 문서는 **향후 분력이 완벽히 가능한 모듈화 구조**를 갖추고 있습니다. 문서 규모가 2,000줄 이상으로 증가하거나, 다중 AI 협업 시 충돌이 발생할 경우 분리를 권장합니다.
...
### 16.10 [v34.1.41] Tkinter 기반 유동적 동적 레이아웃(Flow Layout) 및 지능형 패턴 검출 무결성
- **증상**: 규칙/패턴이 늘어나도 UI 높이가 고정되어 항목이 가려지거나, 제안된 패턴이 닫는 괄호를 누락하여 파일명이 비정상적으로 변경됨.
- **원인 분석**:
    1. **UI 레이아웃**: `grid` 전파 제어 미흡 및 픽셀 기반 동적 높이 계산 로직 부재.
    2. **패턴 검출**: 불완전 정규식 및 특정 괄호(')') 접미사 하드코딩으로 인한 논리 정합성 결여.
- **해결 방안 (Hardening)**:
    1. **Pixel-Perfect Scaling**: `bbox` 좌표 측정 엔진을 도입하여 아이템 추가 시마다 패널 높이를 1px 단위로 즉시 확장하고, 임계치(120px) 도달 시 독립 스크롤바를 자동 활성화.
    2. **Intelligent Pattern Engine**: 다중 괄호((), [], {}) 정밀 매칭 및 파일/폴더 모드별 확장자 자동 제외 로직을 적용하여 제안 패턴의 신뢰도를 99% 이상으로 배양.
    3. **Suffix Dynamic Binding**: 하드코딩된 접미사를 제거하고 엔진이 추출한 완성형 패턴을 그대로 사용하도록 인터랙션 무결성 확보.
    4. **Safety State Capture**: 워커 스레드 시작 전 UI 변수(모드, 재귀 여부)를 사전 캡처(Capture)하여 비동기 실행 시의 데이터 불일치 및 가시성 오류 해결.
    5. **Metadata Hardening**: 개별 항목 추가 시 `_target_mode` 누락을 원천 차단하여 폴더 모드 미리보기 목록 누락 결함 전수 수리.

### 16.11 [v34.1.41] 미리보기 목록 누락 결함 전수 점검 및 해결 타임라인
- **문제 현상 (Incident Statement)**: 
    - 스캔 로그에는 "X개 추가 완료" 및 "Widget 보유 ID: X개"가 표시되나, 실제 UI Treeview 영역은 공백으로 유지되는 현상 지속 발생.
- **오류 해결 이력 및 단계별 조치 (Chronological Resolution Path)**:
    1.  **[1단계] 엔진 재귀 탐색 무결성 수리**: `os.walk`의 딥 스캔 로직에서 하위 폴더 재귀 탐색 상태가 비동기 스레드에서 유실되는 현상 방지 (로컬 변수 캡처 적용).
    2.  **[2단계] 경로 규격 정규화 (Normalization)**: 윈도우 filedialog의 `/`와 os.path의 `\` 충돌로 인한 Treeview IID(아이디) 매칭 실패 해결 (`os.path.normpath` 전수 도입).
    3.  **[3단계] UI 렌더링 강제 확정 (Visibility Force)**: 데이터 삽입 후 Tkinter 메인 루프가 화면을 갱신하지 못하는 현상을 해결하기 위해 `root.update()` 및 물리적 높이(`height=15`) 강제 부여.
    4.  **[4단계] 코드 자가 참조 오류(AttributeError) 수리**: View 클래스 내부에서 `self.status_var`를 `self.view.status_var`로 잘못 지칭하여 스캔 직후 함수가 중단되던 런타임 버그 제거.
    5.  **[5단계] 물리적 레이아웃 차폐 해결**: PanedWindow의 구분선(Sash)이 목록 영역을 압착하여 숨기는 현상을 해결하기 위해 `sash_place` 초기 위치 강제 고정 및 스타일 엔진 단순화.
- **현 시점 점검 결과 (Status)**:
    - **통과된 항목**: 데이터 구조 무결성, 경로 규격 일치, 비동기 스레드 동기화, 물리적 위젯 공간 확보.
    - **잔류 의심 포인트**: 특정 윈도우 테마/해상도 환경에서의 Tkinter Treeview 렌더링 가비지 컬렉션 혹은 테마 충돌 가능성 지속 추적 중.
- **향후 대응 원칙**:
    - 동일한 경로 및 로직 문제에 대한 반복 검토를 지양하고, 위 타임라인에 명시된 해결책을 기반으로 렌더링 엔진 자체의 하드닝에 집중함.
    - **[6단계] 파손 코드 복구 및 잔존 참조 오류 근절 (v34.1.41-R, 2026-04-09)**:
        - 이전 세션에서 레이아웃 간소화 시도(Pack 기반) 중 저장이 불완전하여 `navigate_to_path` 메서드가 중간에 절단(Truncated)되고, 중복된 `_set_style`/`_build_ui` 코드가 삽입되어 구문 에러(SyntaxError) 발생.
        - bb07 백업 기반으로 원래의 Grid+PanedWindow 레이아웃을 완전 복원하고 v34.1.41 하드닝 패치 병합.
        - **View 내부 `self.view.log()` 자기참조 오류 제거**: `update_tree()` 내에서 `self.view.log()`를 `self.log()`로 수정 (View 클래스 내부 메서드이므로 `self.view` 접근 불가).
        - **`rules_canvas` 인스턴스 변수 누락 수정**: `_build_ui()`에서 생성한 `rules_canvas`가 로컬 변수로만 존재하여 `clear_patterns`/`add_pattern_chip`/`clear_rules`/`add_rule_chip`에서 `self.rules_canvas` 접근 시 `AttributeError` 발생. 인스턴스 변수(`self.rules_canvas`)로 승격.

### 3.69 [File Management] 지능형 파일 관리기 긴급 복구 및 참조 오류 근절 (v34.1.41-R)
- **요구사항**: 이전 세션에서 UI 레이아웃 간소화 시도 중 코드 저장이 불완전하여 스크립트 자체가 실행 불가능한 상태를 복구.
- **근본 원인 분석 (Root Cause)**:
    1. **코드 절단(Truncation)**: `navigate_to_path` 메서드 811행에서 코드가 물리적으로 잘리고, 그 뒤에 간소화 시도 중이던 Pack 기반 `_set_style`/`_build_ui` 코드가 삽입됨 → 구문 에러(SyntaxError) 유발.
    2. **View 자기 참조 오류**: `FileOrganizerView.update_tree()` 내부에서 `self.view.log()`를 호출하여 `AttributeError` 발생 (View 내부에서는 `self.log()`가 올바름).
    3. **로컬 변수 누락**: `_build_ui()` 내 `rules_canvas`가 로컬 변수로만 생성되어, 패턴/규칙 칩 메서드에서 `self.rules_canvas` 접근 시 `AttributeError` 발생.
- **해결 조치**:
    1. bb07 백업 기반 Grid+PanedWindow 레이아웃 완전 복원 + v34.1.41 하드닝 패치 병합.
    2. `self.view.log()` → `self.log()` 수정 (1건).
    3. `rules_canvas` → `self.rules_canvas` 인스턴스 변수 승격 (8건).
    4. 수정 전 bb08 백업 생성 완료.
- **수행 도구**: `intelligent_file_organizer.py` **[v34.1.41-R]**
- **상태**: 긴급 복구 및 잔존 참조 오류 근절 완료 (2026-04-09)

---

### 3.70 [Optimizer/PPT] 파워포인트 전용 압축기 3세대 완전 융합 (ppt_image_compression_batch.py v39.4.0)
- **요구사항**: 
    - 기존 1세대 `ppt_compressor_tool.py`의 결함을 극복하고 대시보드(`00 dashboard.html`) 구조와 100% 매칭 융합.
    - 깊은 하위 폴더 전수 스캔 및 트리 유지/플랫 저장 듀얼 모드 지원.
    - 슬라이드 내 **그룹화(`grpSp`) 셰이프**, **도형 채우기(`sp//a:blipFill`)**, **표/배경 미디어**까지 누락 없이 100% 탐색 및 압축.
    - 비대칭 이미지 크기 조정에 따른 렌더링 시 **이중 왜곡(Double Distortion) 및 화질 훼손 원천 차단** (원본 종횡비 100% 보존 스케일링).
    - 단일 폴더 플랫 저장 시 동명 파일 간 덮어쓰기 방지를 위한 자동 순번 접미사(`_1`, `_2`...) 부여.
    - 잘린 그림 영역(`a:srcRect`) 영구 소거 및 크롭 이상치(`l+r >= 100000`) 방어, 용량 역주행 방지 가드.
    - 한글 인코딩 오류 방지, COM 팝업 억제, Windows 파일 락 방탄 재시도 삭제 프로토콜 구비.
- **핵심 아키텍처 및 구현 (Hyper Compression & Aspect-Ratio Preserving Engine)**:
    - **Recursive Group Scaling (`grpSp`) & BlipFill Coverage**:
      - `p:grpSpPr/a:xfrm`의 외부 크기(`ext`)와 자식 좌표계(`chExt`) 배율을 누적 연산(`sub_scale *= ext / chExt`)하여 다중 중첩 그룹 내부 이미지의 실제 표시 픽셀을 정밀 계산.
      - `has_sub_grp` 분기 제어로 상위 컨테이너(`p:spTree`, `p:cSld`)의 조기 탐색을 방지하고, 단일 그림뿐만 아니라 도형 채우기(`p:sp`), 표 셀(`a:tc`), 배경(`p:bg`) 등 모든 `a:blipFill`을 전수 압축.
    - **Aspect Ratio Preservation (Zero Double-Distortion)**:
      - 물리적 이미지 파일 자체는 **고유 Aspect Ratio를 100% 보존**하면서 화면에 필요한 Bounding Box 크기(`target_scale = min(1.0, max(w/orig_w, h/orig_h))`)로만 리사이징하여, Office XML 렌더링 시 가로세로가 2배로 찌그러지는 이중 왜곡을 완벽 차단.
    - **Dual Output Mode (Tree-Preserving vs Flat Root)**: `preserve_tree: bool = True` 옵션을 탑재하여 대상 폴더의 하위 폴더 트리를 100% 동일하게 복제하는 모드와, 트리를 반영하지 않고 출력 폴더 루트에 일괄 모으는 단일 폴더 모드를 제공.
    - **Collision Defense with Suffix**: 플랫 모드 및 기존 파일 존재 시, 디스크 검사뿐만 아니라 작업 내 예약 경로 집합(`allocated_paths`)을 추적하여 `[파일명]_[순번].[확장자]` 형태로 유일 경로를 확정.
    - **True Atomic Replace (Zero Bak Waste)**: `keep_original=False` 설정 시, 임시 파일(`_tmp_...`)에서 압축 및 ZIP CRC 무결성 검증 통과 후 `shutil.move`로 원본을 원자적으로 교체. 불필요한 `.bak` 잔류 파일을 일절 남기지 않음.
    - **Move Mode with Safe Purge**: 출력 폴더를 지정한 상태에서 `keep_original=False` 시, 결과물 생성 확인 후 원본 파일(`.ppt`, `.pptx` 전수)을 `safe_delete_file`로 소거하여 완벽한 "출력 폴더로 이동" 달성.
    - **Anti-Hang COM & Lock Breaking**: `kill_zombie_powerpoint()`로 작업 전 잔류 프로세스를 청소하고 `DispatchEx` + `DisplayAlerts = 0` + `WindowState = 2`로 백그라운드 변환. Windows 파일 락에 대비한 5회 재시도(0.2초 간격) 삭제 프로토콜 적용.
    - **Encoding Armor & Stealth Launch**: `configure_utf8()`을 통해 `sys.stdout/stderr`를 UTF-8로 강제 재구성하고, 이모지를 배제한 텍스트 마커 표준화 및 UTF-8-SIG(BOM) 보고서 생성. `root.lift()`, `root.focus_force()` 화면 전면 가시성 보장.
- **수행 도구**: `ppt_image_compression_batch.py` **[v39.4.0]** (구 `ppt_compressor_tool.py` 대체 융합 배포)
- **상태**: 01 Scripts 대시보드 융합 및 3대 시스템 가이드 동기화 완료 (2026-09-18)

---

### 3.71 [Optimizer/Excel] 엑셀 전용 압축기 3세대 완전 융합 (excel_image_compression_batch.py v39.4.0)
- **요구사항**:
    - `ppt_image_compression_batch.py`와 완전히 동일한 아키텍처, 듀얼 출력 모드, Dark Modern UI 레이아웃으로 엑셀(`xls`, `xlsx`) 전용 압축기 구축.
    - 대시보드(`00 dashboard.html`) 내 '엑셀 전용 압축기' 항목과 100% 매칭 융합.
    - 워크시트 내부의 **그룹화(`xdr:grpSp`) 셰이프**, **도형 이미지 채우기(`xdr:sp//a:blipFill`)**, **표/차트 미디어**까지 전수 탐색 및 압축.
    - 비대칭 이미지 왜곡 조정에 따른 **이중 왜곡(Double Distortion) 및 화질 훼손 원천 차단** (고유 종횡비 100% 보존 리사이징).
    - 단일 폴더 플랫 저장 시 `allocated_paths` 기반 자동 순번 접미사(`_1`, `_2`...) 부여로 덮어쓰기 방어.
    - 원본 보존 해제 시 검증 통과 후 `safe_delete_file()`을 통한 잔재 없는 원자적 In-place Replace 트랜잭션.
- **핵심 아키텍처 및 구현 (DrawingML Group Scaling & Aspect-Ratio Preservation)**:
    - **Recursive Group Scaling (`xdr:grpSp`) & Universal Crop Erasure**:
      - 드로잉 XML에서 `xdr:grpSpPr/a:xfrm`의 배율(`ext / chExt`)을 탑다운 재귀 순회하여 자식 요소에 누적 적용.
      - `has_sub_grp` 검사로 상위 앵커(`twoCellAnchor`, `oneCellAnchor`)의 조기 blip 소비를 차단하고, 도형 채우기(`xdr:sp`) 및 고립 미디어(`xl/media/`)까지 전수 포괄.
      - `root.iter()` 전수 순회로 그룹 내외를 막론하고 크롭된 미디어의 `a:srcRect` 노드를 영구 삭제(`fill.remove(src_rect)`).
    - **Aspect Ratio Preservation (Zero Double-Distortion)**:
      - 물리적 이미지 파일 자체는 **고유 Aspect Ratio를 100% 보존**하면서 화면 목표 해상도(`target_scale = min(1.0, max(w/orig_w, h/orig_h))`)로만 축소하여 엑셀 렌더링 시 찌그러짐 현상 원천 차단.
    - **Dual Output Mode (Tree vs Flat)**:
      - 대상 폴더 하위 트리를 100% 그대로 복제하여 출력하는 '트리 유지 모드'와, 모든 하위 파일을 출력 폴더 루트에 단일 파일로 수집하는 '플랫 저장 모드' 지원.
    - **Collision Defense with Suffix**:
      - 단일 폴더 저장 시 동일 파일명 충돌을 방지하기 위해, 디스크 존재 여부와 함께 작업 세션 내 예약 경로(`allocated_paths`)를 검사하여 `_1`, `_2`, `_3` 접미사를 자동 부여.
    - **True In-place Atomic Replace (Zero Bak Waste)**:
      - `keep_original=False` 설정 시 `_tmp_...xlsx` 격리 파일에서 작업 및 ZIP 무결성 검증 통과 후 `shutil.move`로 원본 교체. 불필요한 `.bak` 잔존 0건 보장.
      - 출력 폴더 지정 시에는 안전 검증 후 원본 파일을 `safe_delete_file()`로 소거하여 완벽한 "이동" 효과 구현.
    - **Excel COM 5대 무인 방어 플래그 & Zombie Purge**:
      - 변환 전 `kill_zombie_excel()`로 잔류 `EXCEL.EXE` 청소.
      - `app.DisplayAlerts = False`, `app.ScreenUpdating = False`, `app.Interactive = False`, `app.WindowState = 2`, `app.AutomationSecurity = 3`를 설정하여 팝업 대화상자 및 매크로 경고 원천 차단.
      - `wb.SaveAs(abs_dest, 51)`를 통해 `.xls`를 현대적 `.xlsx`로 안전하게 승격.
    - **2단계 무결성 자가 검증**:
      - 1단계: `zipfile.ZipFile.testzip()` 및 `xl/workbook.xml` 구조 검사.
      - 2단계: `app.Workbooks.Open()`을 통한 실제 워크시트 카운트 및 열기 검증.
    - **Encoding Armor & Stealth Launch**:
      - `configure_utf8()` 표준 진입점 및 텍스트 마커(`[OK]`, `[FAIL]`, `[변환]`, `[압축]`, `[중복 방지]`, `[원본 정리]`, `[완료]`) 일원화.
      - `root.lift()`, `root.attributes('-topmost', True)`, `root.focus_force()`를 통한 윈도우 전면 가시성 보장.
- **수행 도구**: `excel_image_compression_batch.py` **[v39.4.0]** (구 `excel_compressor_tool.py` 대체 융합 배포)
- **상태**: 01 Scripts 대시보드 융합 및 3대 시스템 가이드 동기화 완료 (2026-09-18)

---

### 3.72 [Closing/Monthly] 월말 마감 파일 패턴폴더 매칭 및 복사·치환 도구 (Monthly_Closing_Tool.py v39.4.0)
- **요구사항**:
    - 대시보드(`00 dashboard.html`) '📅 월말 마감 검토' 섹션의 'B2B 3자간(발주-매입-매출) 교차 검증 및 정제' 카드 우측에 '파일를 폴더로 이동(패턴-치환-표준단가)-' 신규 카드로 융합.
    - 대상 폴더(A)에 취합된 다수의 월마감 파일(`01`~`99` 접두사)을 타겟 상위 폴더(B-1)의 서브폴더(`01 기숙사...`, `02 사택...`) 내 '01 자료'(B-2) 폴더로 정밀 자동 매칭하여 복사.
    - 복사 시 파일명의 앞 2자리 숫자를 사용자 정의 치환 접두사(기본값: `표준단가_`)로 치환하여 저장 (예: `01완료사진_...xlsx` ➔ `표준단가_완료사진_...xlsx`).
    - 타겟 서브폴더 미존재 또는 `01 자료` 부재 시 임의 폴더 생성을 엄격히 금지하고 자동 제외(Skip) 및 원본 보존.
    - 복사 완료 후 전수 매칭 결과를 공시하고 `[원본 정리]`, `[복사본 롤백]`, `[유지 및 완료]` 3대 후속 조치를 선택할 수 있는 대화형 검토 다이얼로그(ReviewDialog) 제공.
- **핵심 아키텍처 및 구현 (Pattern-Matching & Safe Tri-Action Transaction)**:
    - **Dynamic Folder Path Adaptability**:
      - `DEFAULT_SOURCE`(일성) 및 `DEFAULT_TARGET`(09월 솔루션)을 기본값으로 제공하되, 사용자가 매월 자유롭게 대상 폴더와 타겟 상위 폴더를 UI에서 변경할 수 있도록 지원.
      - 외부 JSON 파일 의존성을 0%로 배제하고 메모리 변수 기반으로 안전하게 구동.
    - **Regex Pattern Matching & Destination Isolation**:
      - `re.match(r"^(\d{2})", filename)`를 통해 파일명의 2자리 숫자를 추출하고, 타겟 상위 폴더 내 동일 숫자로 시작하는 서브폴더를 1:1 매칭.
      - 서브폴더 내 `01 자료` 폴더의 물리적 존재를 필수 검증하여 격리 저장 보장.
    - **Atomic File Copy & Size Validation**:
      - `shutil.copy2`로 메타데이터를 보존하며 복사한 후, 원본 파일 크기와 복사본 파일 크기를 바이트 단위로 검증하여 데이터 무결성 확인.
    - **Safe File Deletion Protocol (`safe_delete_file`)**:
      - Windows OS 지연 핸들 락(`PermissionError [WinError 32]`)을 극복하기 위해 `os.chmod(0o777)` 읽기 전용 속성 해제 및 5회 재시도(0.2초 간격) 삭제 프로토콜 적용.
    - **ReviewDialog & Report Export (`utf-8-sig`)**:
      - 복사 완료 즉시 성공/제외 목록을 상세 리포트하고, 필요 시 `[보고서 저장]` 버튼을 통해 한국어 Excel에서 바로 열리는 `utf-8-sig`(BOM) 인코딩 텍스트 파일로 저장 지원.
    - **Focus Hardening for Dashboard Integration**:
      - 대시보드 웹 브라우저에서 Popen 호출 시 창이 숨겨지지 않도록 `lift()`, `attributes('-topmost', True)`, `focus_force()`를 메인 창 및 ReviewDialog에 필수 주입.
- **수행 도구**: `Monthly_Closing_Tool.py` **[v39.4.0]**
- **상태**: 01 Scripts 대시보드 융합 및 3대 시스템 가이드 동기화 완료 (2026-09-18)

---

### 3.73 [System/Maintenance] C·D 드라이브 + Windows 기본 디스크 정리 통합 정기 딥 클린 및 AI 개발환경 보존 자산 명세 (D_Drive_Deep_Clean v3.2.1)
- **요구사항**:
    - C드라이브 SSD의 상시 여유 공간(60~80GB+)을 확보하고 개발 환경 세션 잔여물을 정기적으로 일괄 소거.
    - 대시보드(`00 dashboard.html`) '📁 파일 관리 및 지능형 정리' 섹션에 'C/D 드라이브 + Windows 통합 정기 딥-클린' 카드로 융합.
    - '📘 가이드 및 규정 문서' 섹션에 'Codex·Antigravity D드라이브 이전 마스터 가이드' 카드로 마스터 아키텍처 연동.
    - **Zone A(D: 5대 영역)**, **Zone B(C: 5대 영역)**, **Zone C(Windows cleanmgr 4대 영역)** 총 14개 소모성 캐시 대상을 분리 정밀 청소.
    - `MSoffice` 80개 직속 업무 파일, `.gemini` 대화 DB, `.codex/config.toml` (`gpt-6-astra`), 활성 바이너리는 1 byte도 건드리지 않는 화이트리스트 절대 보호 보증.
    - 휴지통 비우기 시 타 계정/SYSTEM 계정 권한 거부(Access Denied)로 인한 멈춤(Deadlock)을 원천 차단.
    - 삭제 전 시뮬레이션(`--dry-run`) 및 사전 확인 GUI 대화상자 탑재.
- **핵심 아키텍처 및 구현 (Tri-Zone Deep Clean & Absolute Whitelist Guard)**:
    - **3대 Zone 14개 정밀 정리 체계**:
      - **Zone A (D: 드라이브 AI/데이터 캐시 5대)**:
        1. `D:\DevEnv\...\Temp\User` 및 `sessions\temp` (임시 버퍼)
        2. `D:\DevEnv\...\.gemini\antigravity\crashes` (*.log 덤프)
        3. `D:\DevEnv\...\Programs\antigravity-updater\pending` (업데이터 잔여 파일)
        4. `D:\DevEnv\...\AppData\Roaming\TeraBox` (썸네일 및 *.cab 누적 패키지)
        5. `D:\DevEnv\...\Codex\bin` (구버전 해시 디렉터리 및 *.bak, 활성 바이너리 보존)
      - **Zone B (C: 드라이브 OS/스풀/웹 5대)**:
        6. `C:\ProgramData\ESTsoft\ALPDF\PDFCreator` (임시 *.ps 스풀 파일, *.pdf 100% 보존)
        7. `C:\Windows\Temp`, `Local\Temp`, `SoftwareDistribution\Download` (시스템 임시 및 업데이트 다운로드 캐시)
        8. `C:\Windows\Logs\CBS` 누적 로그 및 `C:\ProgramData\Dbg` 진단 심볼/덤프 캐시
        9. Chrome 및 Edge 브라우저 임시 인터넷 캐시
        10. `ms-playwright` 브라우저 캐시 및 NPM 전역 캐시
      - **Zone C (Windows 기본 디스크 정리 4대 cleanmgr 표준)**:
        11. 전송 최적화 파일 (Delivery Optimization Windows OS P2P 캐시, 클라우드 영향 無)
        12. DirectX / NVIDIA 그래픽 셰이더 캐시 (D3DSCache / DXCache)
        13. Windows 오류 보고 (WER) 시스템 덤프 및 크래시 리포트
        14. Windows 휴지통 비우기 (C: 및 D: 드라이브 $Recycle.Bin)
    - **Deadlock-Free User SID Targeting (v3.2.1)**:
      - `Clear-RecycleBin`의 위험한 전체 SID 순회 대신 `whoami /user`를 통해 **현재 로그인 사용자 고유 SID(`S-1-5-21-...-1001`) 폴더만 선별 타깃팅**하여 소거함으로써 SYSTEM 계정(`S-1-5-18`) 권한 거부 데드락을 100% 원천 차단.
      - `$ProgressPreference = 'SilentlyContinue'`로 대량 파일 삭제 시 콘솔 버퍼 지연 완치.
    - **화이트리스트 5대 핵심 자산 절대 보존**:
      - `D:\03 금일작업\00 임시\0000000 MSoffice` (80개 직속 업무 문서 영구 보존).
      - `D:\DevEnv\Relocated-C-Data\UserProfile\.gemini` (AI 대화 DB, SQLite, 25개 스킬 보존).
      - `D:\DevEnv\Relocated-C-Data\UserProfile\.codex` (`config.toml` - `gpt-6-astra` 모델 연동 설정 보존).
      - `D:\DevEnv\Relocated-C-Data\AppData\Local\OpenAI\Codex\bin` (`codex.exe` 등 7대 실행 파일 보존).
      - `C:\ProgramData\ESTsoft\ALPDF\PDFCreator` (46개 `*.pdf` 완성본 문서 보존).
    - **Dual Engine & Simulation Support**:
      - `D_Drive_Deep_Clean.py` (Python 엔진) 및 `D_Drive_Deep_Clean.ps1` (PowerShell 5.1 엔진) 듀얼 제공.
      - `--dry-run` 플래그를 통한 사전 시뮬레이션 및 TopMost GUI 확인 다이얼로그 제공.
    - **Tkinter 다크 모던 GUI 및 큐(Queue) 기반 멀티스레드 완전 격리 아키텍처**:
      - `pythonw.exe` 무콘솔 백그라운드 환경에서도 포커스 하드닝(`lift()`, `topmost`, `focus_force()`)으로 대시보드 클릭 즉시 화면 최상단 팝업.
      - **스레드 분리(Thread Isolation)**: 스레드 시작 전 메인 스레드에서 14대 옵션을 딕셔너리로 스냅샷 복사하여 워커 스레드로 전달. 워커 스레드는 `tk.Variable` 또는 위젯에 일절 접근하지 않고 `queue.Queue`에 이벤트 메시지만 적재. 메인 스레드가 `_poll_ui_queue()`를 통해 모든 위젯 렌더링 및 성과 보고 다이얼로그를 안전하게 전담하여 Python 3.13 `RuntimeError: main thread is not in main loop` 원천 차단.
    - **배치 런처 (`D_Drive_Deep_Clean.bat`) 표준 규격**:
      - Windows 표준 CRLF(`\r\n`) 개행 및 CP949(한국어 ANSI) 인코딩 고정으로 cmd.exe 2바이트 파일 포인터 오프셋 오류 원천 차단.
      - 불안정한 `::` 주석 폐지 및 `REM` 표준화, 기설치된 Python 3.13 경로 직접 매핑 및 UAC 원클릭 자동 승격 보증.
    - **실기 검증(Live Machine Verification) 실측 규격 및 결과**:
      - 3대 Zone 14개 영역 실시간 스캔 통과: 총 3,974개 파일, 약 3.86 GB (3,949.40 MB) 소거 대상 안전 식별.
      - MSOffice 직속 업무 파일 80개 원본 100% 무결 보존 검증 완료.
- **수행 도구**: `D_Drive_Deep_Clean.py` **[v3.2.1 GUI Queue]**, `D_Drive_Deep_Clean.bat` **[v3.2.1 CRLF/CP949]**, `D_Drive_Deep_Clean.ps1` **[v3.2.1]**, `Codex 및 Antigravity D드라이브 이전 가이드.md`
- **상태**: 01 Scripts 대시보드 융합, 멀티스레드 큐 하드닝 및 실기 전수 검증 통과 (2026-09-18)
