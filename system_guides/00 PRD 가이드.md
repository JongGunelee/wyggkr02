...
### 3.69 [Dashboard/Installer] BAT 의존 제거용 설치형 EXE 런처 패키징 (vWeb-1.1)
- **요구사항**:
    - GitHub Pages에서 `bat` 다운로드/실행 없이도 사용자가 **설치형 EXE 1회 실행만으로** 로컬 에이전트를 기동할 수 있도록 배포 구조를 개편.
    - Python 미설치 환경에서도 로컬 런처와 번들 스크립트가 자체적으로 동작하도록 패키징.
- **핵심 구현 (Installer Packaging Refactoring)**:
    - **Launcher Entrypoint**: `dashboard_agent_launcher.py`를 추가하여 실행 시 `웹접속 주소.txt`를 읽고, 기존 에이전트가 살아 있으면 웹 대시보드만 열고, 아니면 로컬 서버를 띄운 뒤 GitHub Pages 주소를 자동 오픈하도록 구성.
    - **Frozen Runtime Bootstrap**: `run_dashboard.py`에 `RUNTIME_ROOT/WORK_ROOT` 개념과 자산 동기화 부트를 도입하여, EXE 내부 번들 리소스를 사용자 로컬 작업 디렉터리로 안전하게 복원 후 실행하도록 전환.
    - **Embedded Script Dispatch**: EXE 환경에서 개별 `.py` 자동화 스크립트 실행 요청 시, 외부 `python.exe` 의존 대신 자기 자신을 `--run-script` 모드로 재호출하여 Python 미설치 환경 호환성을 확보.
    - **PyInstaller Spec Hardening**: `dashboard_agent_launcher.spec`에 `automated_scripts/`, `system_guides/`, `00 dashboard.html`, `service-worker.js`, `manifest.webmanifest`, `웹접속 주소.txt` 및 핵심 의존 모듈 히든임포트를 포함하여 단일 실행파일 빌드를 고정.
    - **Build Path Stabilization**: `build_dashboard_agent.ps1`를 통해 ASCII 임시 빌드 경로(`C:\Users\Public\Documents\ESTsoft\CreatorTemp\wyggkr02_build`)에서 빌드 후 최종 산출물을 `WYGGKR02_Dashboard_Agent_Setup.exe`로 복사하도록 표준화.
    - **Dashboard Download Switch**: `00 dashboard.html`의 설치/재시도 안내와 다운로드 링크를 모두 `WYGGKR02_Dashboard_Agent_Setup.exe` 기준으로 전환.
- **검증**:
    - PyInstaller 빌드 완료: `WYGGKR02_Dashboard_Agent_Setup.exe` 생성 확인.
    - 실기동 검증: 설치형 EXE 실행 후 `http://127.0.0.1:8501/health` 응답 `200 OK` 확인.
- **대상 파일**: `00 dashboard.html`, `run_dashboard.py`, `dashboard_agent_launcher.py`, `dashboard_agent_launcher.spec`, `build_dashboard_agent.ps1`
- **상태**: 완료 (2026-04-07)

---

### 3.70 [Dashboard/Repo] 개발 보관 파일 `dev_source/` 분리 및 루트 배포 정리 (vWeb-1.2)
- **요구사항**:
    - GitHub Pages 업로드 루트가 배포 전용 자산만 포함하도록 단순화하고, 개발/보관용 파일은 별도 폴더로 분리.
    - 파일 이동 이후에도 소스 실행, EXE 재빌드, 레거시 BAT 수동 실행이 깨지지 않도록 경로 정합성을 유지.
- **핵심 구현 (Repository Hygiene Refactoring)**:
    - **Dev Source Isolation**: `run_dashboard.py`, `dashboard_agent_launcher.py`, `dashboard_agent_launcher.spec`, `build_dashboard_agent.ps1`, `웹접속 주소.txt`, `000 Launch_dashboard.bat`를 `dev_source/`로 이동하여 루트에는 웹 배포 자산과 최종 EXE만 남기도록 정리.
    - **Source Root Rebinding**: `run_dashboard.py`에 소스 실행용 프로젝트 루트 탐색기를 추가하여, 스크립트가 `dev_source/` 아래 있어도 실제 작업 루트는 상위 배포 폴더를 사용하도록 보정.
    - **Build Path Rebinding**: `build_dashboard_agent.ps1`와 `dashboard_agent_launcher.spec`를 `dev_source/` 기준으로 수정하여 상위 루트 자산을 수집하고 최종 EXE는 저장소 루트에 출력하도록 고정.
    - **Legacy BAT Compatibility**: `000 Launch_dashboard.bat`의 상대 경로를 `dev_source/` 구조 기준으로 재조정하여 보관용 런처도 수동 실행 가능 상태를 유지.
    - **Address Asset Recovery**: 이동 과정에서 누락될 수 있는 `웹접속 주소.txt`를 `dev_source/`에 복원하여 런처 설정값 일관성 유지.
- **검증**:
    - 소스 경로 검증: `run_dashboard.WORK_ROOT`가 저장소 루트를 가리키는지 확인.
    - EXE 재빌드 검증: `dev_source/build_dashboard_agent.ps1` 실행 후 `WYGGKR02_Dashboard_Agent_Setup.exe` 재생성 확인.
    - EXE 실기동 검증: 재빌드 후 `http://127.0.0.1:8501/health` 응답 `200 OK` 확인.
- **대상 파일**: `dev_source/run_dashboard.py`, `dev_source/dashboard_agent_launcher.py`, `dev_source/dashboard_agent_launcher.spec`, `dev_source/build_dashboard_agent.ps1`, `dev_source/000 Launch_dashboard.bat`, `dev_source/웹접속 주소.txt`
- **상태**: 완료 (2026-04-07)

---

### 3.71 [Dashboard/Installer] tkinter GUI 카드 실행 누락 모듈 보강 (vWeb-1.2.1)
- **요구사항**:
    - 웹 대시보드의 카드 섹션에서 `로컬 실행` 클릭 시, EXE 내부에서 구동되는 Tk 기반 GUI 스크립트가 `No module named 'tkinter'` 오류 없이 정상 기동해야 함.
- **핵심 구현 (Tk Runtime Packaging Fix)**:
    - **Tk Hidden Imports**: `dev_source/dashboard_agent_launcher.spec`에 `tkinter`, `tkinter.ttk`, `tkinter.filedialog`, `tkinter.messagebox`, `tkinter.simpledialog`, `_tkinter`를 명시적으로 추가하여 동적 `runpy` 실행 스크립트에서도 Tk 런타임이 누락되지 않도록 보강.
    - **Hook Activation Verification**: PyInstaller 재빌드 시 `hook-tkinter.py`, `hook-_tkinter.py`, `pyi_rth__tkinter.py`가 실제로 로드되는지 빌드 로그에서 확인.
- **검증**:
    - EXE 재빌드 완료 후 `WYGGKR02_Dashboard_Agent_Setup.exe --run-script automated_scripts/advanced_excel_rename.py` 기동 시 조기 종료 없이 GUI 프로세스 유지 확인.
- **대상 파일**: `dev_source/dashboard_agent_launcher.spec`, `WYGGKR02_Dashboard_Agent_Setup.exe`
- **상태**: 완료 (2026-04-07)

---

### 3.72 [Dashboard/Deploy] 대용량 EXE를 GitHub Release 자산으로 분리 (vWeb-1.2.2)
- **요구사항**:
    - GitHub Pages/저장소 루트의 용량 제한으로 `WYGGKR02_Dashboard_Agent_Setup.exe`를 직접 업로드할 수 없는 환경에서도 다운로드 동선이 유지되어야 함.
- **핵심 구현 (Release Asset Routing)**:
    - **Release Download URL**: `00 dashboard.html`의 EXE 다운로드 버튼을 저장소 상대경로가 아닌 `https://github.com/jonggunelee/wyggkr02/releases/latest/download/WYGGKR02_Dashboard_Agent_Setup.exe` 고정 경로로 전환.
    - **Source Link Rebinding**: 개발 보관 파일 이동 이후 `로컬 에이전트 보기` 링크가 `dev_source/run_dashboard.py`를 가리키도록 수정.
    - **Manual Run Hint Update**: 오프라인 안내문의 수동 실행 예시를 `python dev_source/run_dashboard.py`로 갱신.
- **검증**:
    - HTML 내 인라인 스크립트 문법 검증 완료.
    - 다운로드/소스 링크가 Release 및 `dev_source` 경로를 참조하는지 문자열 수준 확인 완료.
- **대상 파일**: `00 dashboard.html`
- **상태**: 완료 (2026-04-07)

---

### 3.73 [Dashboard/Launcher] EXE 실행 시 브라우저 재활성화 제거 및 기존 탭 혼선 방지 (vWeb-1.2.3)
- **요구사항**:
    - 사용자가 이미 웹 대시보드를 띄운 상태에서 테스트용으로 EXE를 다시 실행하더라도, 브라우저 탭이 새로 열리거나 기존 작업 흐름이 흔들리지 않아야 함.
- **핵심 구현 (Launcher Focus Neutralization)**:
    - **No Auto Browser Reopen**: `dev_source/dashboard_agent_launcher.py`에서 `webbrowser.open()` 및 대기 스레드 기반 자동 웹 오픈 로직을 제거하여, EXE는 로컬 에이전트만 기동하고 브라우저 포커스는 건드리지 않도록 수정.
    - **Existing Session Respect**: 이미 `127.0.0.1:8501` 에이전트가 동작 중이면 새 브라우저를 띄우지 않고 즉시 복귀하도록 유지.
    - **Single Instance Gate**: 런처 프로세스 중복 진입을 완화하기 위해 윈도우 전역 뮤텍스(`Global\\WYGGKR02_Dashboard_Agent_Launcher`)를 도입.
    - **Release Artifact Refresh**: 변경 반영 후 `WYGGKR02_Dashboard_Agent_Setup.exe` 및 `WYGGKR02_Dashboard_Agent_Setup.zip` 재생성.
- **검증**:
    - PyInstaller 재빌드 완료.
    - 최신 EXE/ZIP 산출물 생성 시각 갱신 확인.
- **대상 파일**: `dev_source/dashboard_agent_launcher.py`, `WYGGKR02_Dashboard_Agent_Setup.exe`, `WYGGKR02_Dashboard_Agent_Setup.zip`
- **상태**: 완료 (2026-04-08)
