...
XPR_DL_API xpr_rcode_t xpr_malloc(void **aAddr, xpr_size_t aSize)
{
    ...
    // [Win11 Optimization] CRITICAL BUG FIX: Condition was inverted!
    // GetProcessHeap() returns NULL on failure. The original code
    // returned error when heap was VALID (!=NULL), causing all
    // allocations to silently fail or use invalid heap.
    if (sHeap == XPR_NULL)
...
XPR_DL_API xpr_rcode_t xpr_calloc(void **aAddr, xpr_size_t aNumber, xpr_size_t aSize)
{
    ...
    HANDLE sHeap = ::GetProcessHeap();
    // [Win11 Optimization] CRITICAL BUG FIX: Same inverted condition fix.
    if (sHeap == XPR_NULL)
        return XPR_RCODE_GET_OS_ERROR();
