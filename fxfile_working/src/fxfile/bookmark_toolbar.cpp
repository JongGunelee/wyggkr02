...
enum
{
    ...
    WM_BOOKMARK_LOAD_ICONS = WM_USER + 101,
};
} // namespace anonymous
...
BEGIN_MESSAGE_MAP(BookmarkToolBar, super)
    ...
    ON_MESSAGE(WM_BOOKMARK_LOAD_ICONS, OnBookmarkLoadIcons)
END_MESSAGE_MAP()
...
LRESULT BookmarkToolBar::OnBookmarkAsyncIcon(WPARAM wParam, LPARAM lParam)
{
    ...
    if (mImgList.m_hImageList != XPR_NULL &&
        XPR_IS_RANGE(0, sIndex, BookmarkMgr::instance().getCount()-1))
    {
...
void BookmarkToolBar::setBookmark(void)
{
    ...
    if (mImgList.m_hImageList != XPR_NULL)
    {
        GetToolBarCtrl().SetImageList(XPR_NULL);
        mImgList.DeleteImageList();
    }

    // CImageList::Create requires non-negative capacities.  The old -1/-1
    // values can make ImageList_Create fail, leaving bookmark buttons with
    // text only.  Reserve both the pending icons and their async replacements.
    xpr_sint_t sImageCount = XPR_MAX(1, sCount * 2);
    xpr_sint_t sImageGrow  = XPR_MAX(1, sCount);
    xpr_bool_t sCreated = mImgList.Create(16, 16, ILC_COLOR32 | ILC_MASK, sImageCount, sImageGrow);
    if (sCreated == XPR_FALSE)
        sCreated = mImgList.Create(16, 16, ILC_COLOR16 | ILC_MASK, sImageCount, sImageGrow);

    if (sCreated == XPR_TRUE)
    ...
    // Icon extraction may invoke Shell extensions and antivirus scanning.
    // Let LoadFrame finish and show the real 2x2 window first, then resolve
    // local icons synchronously from this toolbar's own posted message.  Slow
    // network bookmarks remain on BookmarkMgr's asynchronous path.
    ++mIconLoadGeneration;
    PostMessage(WM_BOOKMARK_LOAD_ICONS, (WPARAM)mIconLoadGeneration, 0);
}

LRESULT BookmarkToolBar::OnBookmarkLoadIcons(WPARAM wParam, LPARAM lParam)
{
    if ((xpr_uint_t)wParam != mIconLoadGeneration || mImgList.m_hImageList == XPR_NULL)
        return 0;
    ...
    {
        BookmarkItem *sBookmarkItem = sBookmarkMgr.getBookmark(i);
        if (XPR_IS_NULL(sBookmarkItem) || XPR_IS_TRUE(sBookmarkItem->isSeparator()))
            continue;

        HICON sIcon = XPR_NULL;
        sBookmarkMgr.getIcon(i, sIcon, XPR_TRUE);
