...
void ContentsWnd::OnPaint(void) 
{
    ...
    {
        COLORREF sBackColor = CalculateColor(RGB(255,255,255), GetSysColor(COLOR_3DFACE), 180);
    ...
    {
        COLORREF sBackColor = CalculateColor(RGB(255,255,255), GetSysColor(COLOR_3DFACE), 200);
...
void ContentsWnd::setContentsNormal(LPTVITEMDATA aTvItemData, xpr_bool_t aUpdate)
{
    ...
    GetFolderPath(aTvItemData->mShellFolder, aTvItemData->mPidl, sPath, aTvItemData->mShellAttributes);
                                        ...
                                        const xpr_tchar_t *aAttr)
{
    ...
    if (IsVirtualItem(aLvItemData->mShellFolder, aLvItemData->mPidl))
    {
        xpr_tchar_t sInfotip[INFOTIPSIZE + 1] = {0};
        if (GetInfotip(m_hWnd, aLvItemData->mShellFolder, aLvItemData->mPidl, sInfotip))
            mDispDeque.push_back(new InfotipItem(sInfotip));
    }
                                   ...
                                   xpr_bool_t    aUpdate)
{
    ...
    {
        xpr_tchar_t sInfotip[INFOTIPSIZE + 1] = {0};
        if (GetInfotip(m_hWnd, aShellFolder, aPidl, sInfotip))
            mDispDeque.push_back(new InfotipItem(sInfotip));
    }
    ...
    xpr_tchar_t sPath[XPR_MAX_PATH + 1] = {0};
    GetFolderPath(aShellFolder, aPidl, sPath, aAttributes);

    xpr_uint64_t sTotalSize = 0;
    xpr_uint64_t sUsedSize  = 0;
    xpr_uint64_t sFreeSize  = 0;
    GetDiskSize(sPath, sTotalSize, sUsedSize, sFreeSize);
...
void ContentsWnd::updateFont(void)
{
    ...
    if (sDc != XPR_NULL)
    {
        sLogFont.lfHeight = -MulDiv(13, GetDeviceCaps(sDc->m_hDC, LOGPIXELSY), 72);
        ReleaseDC(sDc);
    }
