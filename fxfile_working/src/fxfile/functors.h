...
{
    std::for_each(aContainer.begin(), aContainer.end(), fxfile::HeapDeleteFunctor());
    aContainer.clear();
}
...
{
    std::for_each(aContainer.begin(), aContainer.end(), fxfile::MapHeapDeleteFunctor());
    aContainer.clear();
}
} // namespace fxfile
