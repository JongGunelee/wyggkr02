...
namespace
{
const xpr_sint_t kPaneWidth[] = { 120, 80, 60, 80, 0 };
...
xpr_sint_t ExplorerStatusBar::OnCreate(LPCREATESTRUCT aCreateStruct)
{
    ...
    setPaneSize(4, kPaneWidth[4], kPaneWidth[4]);
    ...
    SetTimer(IDT_CLOCK_TIMER, 1000, XPR_NULL);
    updateClock();
...
void ExplorerStatusBar::updateClock(void)
{
    SYSTEMTIME st;
    ::GetLocalTime(&st);

    const xpr_tchar_t *sWeekDays[] = {
        XPR_STRING_LITERAL("일"), XPR_STRING_LITERAL("월"), XPR_STRING_LITERAL("화"),
        XPR_STRING_LITERAL("수"), XPR_STRING_LITERAL("목"), XPR_STRING_LITERAL("금"),
        XPR_STRING_LITERAL("토")
    };
    const xpr_tchar_t *sAmPm = (st.wHour < 12) ? XPR_STRING_LITERAL("오전") : XPR_STRING_LITERAL("오후");
    xpr_sint_t sHour12 = st.wHour % 12;
    if (sHour12 == 0) sHour12 = 12;

    xpr_tchar_t sClockText[128] = {0};
    _stprintf(sClockText, XPR_STRING_LITERAL("%04d-%02d-%02d (%s) %s %02d:%02d:%02d"),
              st.wYear, st.wMonth, st.wDay, sWeekDays[st.wDayOfWeek % 7], sAmPm, sHour12, st.wMinute, st.wSecond);

    setPaneText(4, sClockText);
}

void ExplorerStatusBar::OnTimer(UINT_PTR nIDEvent)
{
    if (nIDEvent == IDT_CLOCK_TIMER)
    {
        updateClock();
        return;
    }
