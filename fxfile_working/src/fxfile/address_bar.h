...
public:
    ...
    // Startup and ExplorerPane synchronization only need to show the current
    // path.  Building the complete Desktop/Computer drop-down tree here made
    // every pane repeat the same expensive shell enumeration before the saved
    // four-pane layout became usable.  The full tree is initialized lazily by
    // OnDropdown() or by an explicit address-bar command.
    xpr_bool_t showCurrentPath(LPITEMIDLIST aFullPidl);
