...
{
    ShellChangeNotify::instance().unregisterWatch(mShcnDesktopId);
    ShellChangeNotify::instance().unregisterWatch(mShcnComputerId);
    mShcnDesktopId = 0;
    mShcnComputerId = 0;

    MSG sMessage = {0};
    while (::PeekMessage(&sMessage, m_hWnd,
                         WM_SHCN_DESKTOP, WM_SHCN_DESKTOP, PM_REMOVE))
    {
        Shcn *sShcn = reinterpret_cast<Shcn *>(sMessage.wParam);
    ...
    while (::PeekMessage(&sMessage, m_hWnd,
                         WM_SHCN_COMPUTER, WM_SHCN_COMPUTER, PM_REMOVE))
    {
        Shcn *sShcn = reinterpret_cast<Shcn *>(sMessage.wParam);
        if (XPR_IS_NOT_NULL(sShcn))
        {
...
xpr_bool_t AddressBar::showCurrentPath(LPITEMIDLIST aFullPidl)
{
    ...
    // ComboBoxEx owns a separate edit control.  Updating that control keeps
    // the visible address identical without forcing construction of all
    // Desktop/My Computer items which are invisible until the arrow is used.
    CEdit *sEditCtrl = GetEditCtrl();
    if (XPR_IS_NOT_NULL(sEditCtrl) && XPR_IS_NOT_NULL(sEditCtrl->m_hWnd))
        sEditCtrl->SetWindowText(sFullPath);
    else
        SetWindowText(sFullPath);
...
void AddressBar::ensureBaseItems(void)
{
    if (XPR_IS_TRUE(mInit))
        return;

    LPITEMIDLIST sCurrentFullPidl = fxfile::base::Pidl::clone(mOldSelFullPidl);

    doBaseItems();
    mInit = XPR_TRUE;

    if (XPR_IS_NOT_NULL(sCurrentFullPidl))
    {
        // mInit is already true, so this only adds/selects the current branch
        // and never recurses into the lazy initialization path.
        explore(sCurrentFullPidl);
        COM_FREE(sCurrentFullPidl);
    }
}
...
void AddressBar::setCustomFont(xpr_tchar_t *aFontText)
{
    ...
    LOGFONT sLogFont = {0};
    Option::getScaledFont(sLogFont, aFontText);
...
void AddressBar::setCustomFont(CFont *aFont)
{
    ...
    {
        Option::getScaledFont(sLogFont, XPR_NULL);
    }
