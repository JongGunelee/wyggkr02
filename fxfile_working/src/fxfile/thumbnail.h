...
public:
    ...
    // Empty means the existing ConfDir-based cache location. Changing this
    // value resets the in-memory thumbnail cache but never moves old files.
    void setCacheDir(const xpr_tchar_t *aCacheDir);
    const xpr_tchar_t *getCacheDir(void) const;

    xpr_bool_t loadCache(void);
    xpr_bool_t saveCache(void);
    ...
    void cancelAsyncImage(HWND aHwnd, xpr_uint_t aMsg);
...
protected:
    ...
    ThumbItem *mInFlightThumbItem;
    typedef std::set<HWND> CancelledOwnerSet;
    CancelledOwnerSet mCancelledOwners;
