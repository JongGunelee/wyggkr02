...
{
    PTOKEN_USER sTokenUser = GetUserToken(aTokenHandle);
    if (XPR_IS_NULL(sTokenUser))
        return XPR_FALSE;

    xpr_bool_t sResult = GetTextualSid(sTokenUser->User.Sid, aStringSid, &aStringLen);
    ...
    for (; sIterator != mIndexDeque.end(); ++sIterator)
    {
        ...
        if (_tcsicmp(sIndex->mOriginalFilePath, aOriginalFilePath) == 0)
        {
            if (XPR_IS_TRUE(sIndex->mInfo2Mode))
            {
                ...
                sFileExt = GetFileExt(sIndex->mOriginalFilePath);
