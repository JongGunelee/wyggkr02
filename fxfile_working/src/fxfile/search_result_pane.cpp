...
void SearchResultPane::onUpdatedResultInfo(SearchResultCtrl &aSearchResultCtrl)
{
    ...
    SizeFormat::getFormatedNumber(sResultFileCount,  sResultFileCountText,  XPR_COUNT_OF(sResultFileCountText)  - 1);
    SizeFormat::getFormatedNumber(sResultDirCount,   sResultDirCountText,   XPR_COUNT_OF(sResultDirCountText)   - 1);
    SizeFormat::getFormatedNumber(sResultTotalCount, sResultTotalCountText, XPR_COUNT_OF(sResultTotalCountText) - 1);
    ...
    SizeFormat::getDefSizeFormat(sResultTotalSize, mStatusText1, XPR_COUNT_OF(mStatusText1) - 1);
...
xpr_bool_t SearchResultPane::onOpenFolder(SearchResultCtrl &aSearchResultCtrl, LPCITEMIDLIST aFullPidl)
{
    if (XPR_IS_NULL(mObserver))
        return XPR_FALSE;
