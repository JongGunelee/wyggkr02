...
namespace
{
void deleteThumbImage(ThumbImage *aThumbImage)
{
    if (XPR_IS_NOT_NULL(aThumbImage) &&
        XPR_IS_NOT_NULL(aThumbImage->mImage))
    {
        aThumbImage->mImage->DeleteObject();
        XPR_SAFE_DELETE(aThumbImage->mImage);
    }
    XPR_SAFE_DELETE(aThumbImage);
}
} // namespace anonymous
...
void Thumbnail::create(void)
{
    ...
    mThumbImgList->setCacheDir(mCacheDir.c_str());
...
void Thumbnail::stop(void)
{
    ...
    // Event lifetime ends only after the waiter has exited.  This also makes
    // a later start() create a fresh event without a startup race.
    {
        xpr::MutexGuard sLockGuard(mMutex);
        CLOSE_HANDLE(mEvent);
    }
...
void Thumbnail::setCacheDir(const xpr_tchar_t *aCacheDir)
{
    const xpr_tchar_t *sCacheDir = XPR_IS_NOT_NULL(aCacheDir)
                                ? aCacheDir
                                : XPR_STRING_LITERAL("");
    if (_tcsicmp(mCacheDir.c_str(), sCacheDir) == 0)
        return;

    mCacheDir = sCacheDir;

    // The image list is shared by every ExplorerCtrl. Replacing it while the
    // application is running would leave other panes holding a stale
    // HIMAGELIST. Keep the in-memory generation and only redirect its next
    // save; the selected location is loaded on the next application start.
    if (XPR_IS_NOT_NULL(mThumbImgList))
        mThumbImgList->setCacheDir(mCacheDir.c_str());
}

const xpr_tchar_t *Thumbnail::getCacheDir(void) const
{
...
xpr_bool_t Thumbnail::loadCache(void)
{
    ...
    sThumbImgList->setCacheDir(mCacheDir.c_str());
    ...
    xpr_sint_t sLoadedWidth = 0;
    xpr_sint_t sLoadedHeight = 0;
    if (::ImageList_GetIconSize(sThumbImgList->getImageList()->m_hImageList,
                                &sLoadedWidth, &sLoadedHeight) == FALSE ||
        sLoadedWidth != mThumbSize.cx || sLoadedHeight != mThumbSize.cy)
    {
        XPR_SAFE_DELETE(sThumbImgList);
        return XPR_FALSE;
    }
...
xpr_bool_t Thumbnail::saveCache(void)
{
    if (XPR_IS_NULL(mThumbImgList))
        return XPR_FALSE;

    return mThumbImgList->save();
}
...
void Thumbnail::setThumbSize(CSize aThumbSize)
{
    ...
    {
        const xpr_bool_t sRestart =
            XPR_IS_NOT_NULL(mThread.getThreadHandle().mHandle);
        if (mThread.getThreadState() != xpr::ThreadStateNone)
            stop();

        clearAysncImage();
        mThumbImgList->resize(aThumbSize.cx, aThumbSize.cy);

        mIdMap.clear();
        mIdMgr = 0;

        if (XPR_IS_TRUE(sRestart))
            start();
    }
...
void Thumbnail::initThumb(void)
{
    if (XPR_IS_NOT_NULL(mThumbImgList))
    {
        ...
        // A queued or already decoded thumbnail must not repopulate the cache
        // immediately after the user explicitly initializes it.
        clearAysncImage();
        mThumbImgList->deleteCacheFiles();
...
xpr_uint_t Thumbnail::addImage(ThumbImage *aThumbImage)
{
    ...
    ThumbImgList::ThumbElement *sExisting =
        mThumbImgList->getImage(aThumbImage->mPath.c_str());
    if (XPR_IS_NOT_NULL(sExisting))
        return sExisting->mThumbImageId;
...
xpr_bool_t Thumbnail::getAsyncImage(HWND aHwnd, xpr_uint_t aMsg, xpr_uint_t aCode, const xpr_tchar_t *aPath, xpr_bool_t aLoadByExt)
{
...

    {
        ...
        // LVN_GETDISPINFO can request the same thumbnail repeatedly while a
        // row is repainted.  Keep one request per pane-generation/path/size.
        const ThumbItem *sPending = mInFlightThumbItem;
        if (XPR_IS_NOT_NULL(sPending) &&
            sPending->mHwnd == sThumbItem->mHwnd &&
            sPending->mMsg == sThumbItem->mMsg &&
            sPending->mCode == sThumbItem->mCode &&
            sPending->mThumbSize == sThumbItem->mThumbSize &&
            _tcsicmp(sPending->mPath.c_str(),
                     sThumbItem->mPath.c_str()) == 0)
        {
            XPR_SAFE_DELETE(sThumbItem);
            return XPR_TRUE;
        }

        for (ThumbDeque::const_iterator sIt = mThumbDeque.begin();
             sIt != mThumbDeque.end(); ++sIt)
        {
            sPending = *sIt;
            if (XPR_IS_NOT_NULL(sPending) &&
                sPending->mHwnd == sThumbItem->mHwnd &&
        ...
        // Prefer recent visible rows, but never let rapid navigation grow the
        // decoder queue without bound.
        static const xpr_size_t kMaxPendingThumbnails = 256;
        if (mThumbDeque.size() >= kMaxPendingThumbnails)
        {
            ThumbItem *sOldest = mThumbDeque.back();
            mThumbDeque.pop_back();
            XPR_SAFE_DELETE(sOldest);
        }
...
void Thumbnail::cancelAsyncImage(HWND aHwnd, xpr_uint_t aMsg)
{
    ...
    {
        xpr::MutexGuard sLockGuard(mMutex);
        if (XPR_IS_NOT_NULL(mInFlightThumbItem) &&
            mInFlightThumbItem->mHwnd == aHwnd &&
            mInFlightThumbItem->mMsg == aMsg)
            mCancelledOwners.insert(aHwnd);

        for (ThumbDeque::iterator sIt = mThumbDeque.begin();
             sIt != mThumbDeque.end(); )
        {
            ThumbItem *sItem = *sIt;
            if (XPR_IS_NOT_NULL(sItem) && sItem->mHwnd == aHwnd &&
                sItem->mMsg == aMsg)
            {
                XPR_SAFE_DELETE(sItem);
                sIt = mThumbDeque.erase(sIt);
            }
            else
            {
                ++sIt;
    ...
    // The UI thread owns posted results.  Drain messages that were committed
    // just before cancellation while the HWND is still valid.
    MSG sMessage = {0};
    while (::PeekMessage(&sMessage, aHwnd, aMsg, aMsg, PM_REMOVE))
        deleteThumbImage(reinterpret_cast<ThumbImage *>(sMessage.lParam));
}
...
void Thumbnail::rebuildIdMap(void)
{
    mIdMap.clear();
    mIdMgr = 0;
    ...
    const xpr_sint_t sCount = mThumbImgList->getImageCount();
    xpr_sint_t i;
    for (i = 0; i < sCount; ++i)
    {
        ThumbImgList::ThumbElement *sThumbElement = mThumbImgList->getImage(i);
        if (XPR_IS_NULL(sThumbElement))
            continue;

        mIdMap[sThumbElement->mThumbImageId] = sThumbElement->mImageIndex;
        if (sThumbElement->mThumbImageId >= mIdMgr &&
            sThumbElement->mThumbImageId != InvalidThumbImageId)
        {
...
xpr_sint_t Thumbnail::runThread(xpr::Thread &aThread)
{
    ...
    while (true)
    {
        ...
        if (XPR_IS_NOT_NULL(sThumbImage))
        {
            {
                xpr::MutexGuard sLockGuard(mMutex);
                if (mCancelledOwners.find(sHwnd) != mCancelledOwners.end() ||
                    ::IsWindow(sHwnd) == XPR_FALSE ||
                    ::PostMessage(sHwnd, sMsg,
                                  (WPARAM)sThumbItem->mCode,
                                  (LPARAM)sThumbImage) == XPR_FALSE)
                    deleteThumbImage(sThumbImage);
            }
        }

        // Duplicate suppression reads mInFlightThumbItem under mMutex.  Clear
        // it before deleting the completed request to avoid a dangling read.
        {
            xpr::MutexGuard sLockGuard(mMutex);
            if (mInFlightThumbItem == sThumbItem)
            {
                mInFlightThumbItem = XPR_NULL;
                mCancelledOwners.erase(sHwnd);
            }
        }
        XPR_SAFE_DELETE(sThumbItem);
    }
