...
namespace
{
void getRecursiveFolderSize(const xpr_tchar_t *aFolder,
                            WIN32_FIND_DATA &aWin32FindData,
                            xpr_uint64_t &aFolderSize,
                            HANDLE aStopEvent)
{
    if (XPR_IS_NOT_NULL(aStopEvent) &&
        ::WaitForSingleObject(aStopEvent, 0) == WAIT_OBJECT_0)
        return;
    ...
    if (sFind != INVALID_HANDLE_VALUE)
    {
        do
        {
            ...
            if (!XPR_TEST_BITS(aWin32FindData.dwFileAttributes, FILE_ATTRIBUTE_DIRECTORY))
            {
                ...
                aFolderSize +=
                    (xpr_uint64_t)aWin32FindData.nFileSizeHigh << 32;
            }

            if (XPR_TEST_BITS(aWin32FindData.dwFileAttributes,
                              FILE_ATTRIBUTE_DIRECTORY) &&
                !XPR_TEST_BITS(aWin32FindData.dwFileAttributes,
...
void FolderSize::addFolderSize(const xpr_tchar_t *aDir)
{
...

    {
        ...
        if (XPR_IS_NULL(mThread))
        {
            ...
            mStop  = ::CreateEvent(XPR_NULL, XPR_TRUE, XPR_FALSE, XPR_NULL);
            if (XPR_IS_NULL(mEvent) || XPR_IS_NULL(mStop))
            {
                CLOSE_HANDLE(mEvent);
                CLOSE_HANDLE(mStop);
                return;
            ...
            mThread = (HANDLE)::_beginthreadex(
                XPR_NULL, 0, FolderSizeProc, this, 0, &mThreadId);
            if (XPR_IS_NULL(mThread))
            {
                CLOSE_HANDLE(mEvent);
                CLOSE_HANDLE(mStop);
...
void FolderSize::stop(DWORD aStopMillseconds)
{
    ...
    if (XPR_IS_NOT_NULL(mStop))
        ::SetEvent(mStop);

    DWORD sWait = ::WaitForSingleObject(mThread, aStopMillseconds);
    if (sWait == WAIT_TIMEOUT)
    {
        // Never terminate a CRT worker while it owns heap and find handles.
        // Cancel pending synchronous filesystem I/O, then let its cooperative
        // stop checks unwind the stack and close resources normally.
        ::CancelSynchronousIo(mThread);
...
DWORD FolderSize::OnFolderSize(void)
{
    ...
    while (true)
    {
        ...
        if (sWait == WAIT_OBJECT_0)
        {
            ...
            if (::WaitForSingleObject(mStop, 0) == WAIT_OBJECT_0)
                break;
...
xpr_uint64_t FolderSize::getFolderSize(const xpr_tchar_t *aDir)
{
    ...
    getRecursiveFolderSize(aDir, sWin32FindData, sFolderSize, XPR_NULL);
