...
// IDataObject::GetData gives ownership of the returned STGMEDIUM to its
// caller.  COleDataSource::CacheData stores that complete medium (including
// pUnkForRelease), so detach the local copy after transferring ownership.
static xpr_bool_t CacheClipboardFormat(LPDATAOBJECT    aDataObject,
                                       COleDataSource *aDataSource,
                                       ...
                                       FORMATETC      &aFormatEtc)
{
    ...
    HRESULT sComResult = aDataObject->GetData(&aFormatEtc, &sStgMedium);
    if (FAILED(sComResult))
        return XPR_FALSE;

    if (sStgMedium.tymed == TYMED_NULL)
    {
        ::ReleaseStgMedium(&sStgMedium);
        return XPR_FALSE;
    }

    aDataSource->CacheData(aClipFormat, &sStgMedium, &aFormatEtc);
    memset(&sStgMedium, 0, sizeof(sStgMedium));
...
void SetClipboard(LPDATAOBJECT aDataObject, DROPEFFECT aDropEffect, CLIPFORMAT *aClipFormat)
{
    ...
    // ShellIDList
    CacheClipboardFormat(aDataObject, sDataSource, aClipFormat[0], sFormatEtc);

    // Unknown Clipformat - HDROP
    CacheClipboardFormat(aDataObject, sDataSource, CF_HDROP, sFormatEtc);
    ...
    // FileName
    CacheClipboardFormat(aDataObject, sDataSource, aClipFormat[2], sFormatEtc);

    // FileNameW
    CacheClipboardFormat(aDataObject, sDataSource, aClipFormat[3], sFormatEtc);
                   ...
                   xpr_bool_t         aCopy)
{
    ...
    if (aDropEffectClipFormat > 4)
    {
        ...
        DROPEFFECT *sDropEffectData = (DROPEFFECT *)::GlobalLock(sGlobal);
        if (XPR_IS_NULL(sDropEffectData))
        {
            ::GlobalFree(sGlobal);
            return XPR_FALSE;
        }

        sDropEffect = *sDropEffectData;
        ::GlobalUnlock(sGlobal);
        ::GlobalFree(sGlobal);
    }
    ...
    if (aOleDataObject->IsDataAvailable(aShellIDListClipFormat) == XPR_TRUE && aDropEffect == DROPEFFECT_LINK)
    {
        ...
        if (aOleDataObject->GetData(aShellIDListClipFormat, &sStgMedium, &sFormatEtc) == XPR_TRUE)
        {
            ...
            for (i = 0; i < sCount; ++i)
            {
                ...
                sFullPidl = fxfile::base::Pidl::concat(sFolderPidl, sItemPidl);
    ...
    if (aOleDataObject->IsDataAvailable(aShellIDListClipFormat) == XPR_TRUE)
    {
        ...
        if (aOleDataObject->GetData(aShellIDListClipFormat, &sStgMedium, &sFormatEtc) == XPR_TRUE)
        {
            ...
            for (i = 0; i < (xpr_sint_t)sFiles; ++i)
            {
                // get item PIDL and get full-qualified PIDL
                sItemPidl = (LPITEMIDLIST)(((xpr_byte_t *)sIda)+(sIda)->aoffset[i+1]);
                GetName(sShellFolder, sItemPidl, SHGDN_FORPARSING, sPath);
...
xpr_bool_t DoPasteBitmap(LPDATAOBJECT aDataObject, const xpr_tchar_t *aPath)
{
    ...
    if (SUCCEEDED(sComResult))
    {
        ...
        // hBitmap is borrowed from STGMEDIUM. ReleaseStgMedium either
        // deletes it or delegates to pUnkForRelease; never DeleteObject it
        // directly here.
        ::ReleaseStgMedium(&sStgMedium);
    }
...
xpr_bool_t GetPreferredDropEffect(COleDataObject *aOleDataObject, CLIPFORMAT aPreferredDropEffectClipFormat, DROPEFFECT &aDropEffect)
{
    ...
    DROPEFFECT *sDropEffect = (DROPEFFECT *)::GlobalLock(sGlobal);
    if (XPR_IS_NULL(sDropEffect))
    {
        ::GlobalFree(sGlobal);
        return XPR_FALSE;
    }

    aDropEffect = *sDropEffect;
    ::GlobalUnlock(sGlobal);
    ::GlobalFree(sGlobal);
...
xpr_bool_t DoPasteInetUrl(LPDATAOBJECT aDataObject, xpr_tchar_t *aDir, CLIPFORMAT aInetUrlClipFormat, CLIPFORMAT aFileContentsClipFormat, CLIPFORMAT aFileDescriptorClipFormat, xpr_bool_t aUnicode)
{
    ...
    HRESULT sHResult;
    HRESULT sInetUrlResult = aDataObject->GetData(&sInetUrlFormatEtc, &sInetUrlStgMedium);
    HRESULT sDescriptorResult = aDataObject->GetData(&sDescriptorFormatEtc, &sDescriptorStgMedium);

    if (SUCCEEDED(sInetUrlResult) && SUCCEEDED(sDescriptorResult) && XPR_IS_NOT_NULL(sInetUrlStgMedium.hGlobal) && XPR_IS_NOT_NULL(sDescriptorStgMedium.hGlobal))
    {
        xpr_tchar_t *sUrl;
        ...
        if (XPR_IS_NOT_NULL(sFileGroupDescA) || XPR_IS_NOT_NULL(sFileGroupDescW))
            ::GlobalUnlock(sDescriptorStgMedium.hGlobal);
    }

    // The two GetData calls are independent. Release whichever one
    // succeeded even when the other format was unavailable or malformed.
    if (SUCCEEDED(sInetUrlResult))
        ::ReleaseStgMedium(&sInetUrlStgMedium);

    if (SUCCEEDED(sDescriptorResult))
...
xpr_bool_t DoPasteFileContents(LPDATAOBJECT aDataObject, xpr_tchar_t *aDir, CLIPFORMAT aFileContentsClipFormat, CLIPFORMAT aFileDescriptorClipFormat, xpr_bool_t aUnicode)
{
    ...
    STGMEDIUM sStgMedium = {0};
    HRESULT sDescriptorResult = aDataObject->GetData(&sDescriptorFormatEtc, &sStgMedium);

    if (SUCCEEDED(sDescriptorResult) && XPR_IS_NOT_NULL(sStgMedium.hGlobal))
    {
        if (XPR_IS_TRUE(aUnicode))
    ...
    if ((SUCCEEDED(sDescriptorResult)) && (XPR_IS_NOT_NULL(sFileGroupDescA) || XPR_IS_NOT_NULL(sFileGroupDescW)))
    {
        RemoveLastSplit(aDir);
        ...
        for (i = 0; i < sCount; ++i)
        {
            ...
            memset(&sFileStgMedium, 0, sizeof(sFileStgMedium));
            HRESULT sContentResult = aDataObject->GetData(&sContentsFormatEtc, &sFileStgMedium);
            if (SUCCEEDED(sContentResult))
            {
                const xpr_bool_t sValidContent =
                    (sFileStgMedium.tymed == TYMED_ISTREAM && XPR_IS_NOT_NULL(sFileStgMedium.pstm)) ||
                    (sFileStgMedium.tymed == TYMED_ISTORAGE && XPR_IS_NOT_NULL(sFileStgMedium.pstg));

                if (XPR_IS_TRUE(sValidContent))
                {
                    if (XPR_IS_TRUE(aUnicode))
                    {
                        sInputBytes = wcslen(sFileDescW->cFileName) * sizeof(xpr_wchar_t);
                    ...
                    HRESULT sWriteResult = E_FAIL;
                    switch (sFileStgMedium.tymed)
                    {
                    case TYMED_ISTORAGE:
                        {
                            sWriteResult = StorageToFile(sFileStgMedium.pstg, sPath);
                            break;
                        }

                    case TYMED_ISTREAM:
                        {
                            sWriteResult = StreamToFile(sFileStgMedium.pstm, sPath);
                            break;
                        }
                    }

                    if (SUCCEEDED(sWriteResult))
                    {
                        ::SHChangeNotify(SHCNE_CREATE, SHCNF_PATH | SHCNF_FLUSH, sPath, XPR_NULL);
                        sResult = XPR_TRUE;
                    }
