...
void FileScrap::doOperation(xpr_uint_t aGroupId, xpr_sint_t aMode, const xpr_tchar_t *aTarget)
{
    ...
    bool sSucceeded = false;
    if (FileOperationLockStore::instance().isOperationBlocked(
            &sShFileOpStruct, sBlockedPath))
    {
        std::wstring sMessage = L"FxFile 작업 잠금으로 보호된 경로입니다.\n\n";
        sMessage += sBlockedPath;
        ::MessageBoxW(AfxGetMainWnd() != NULL ?
                      AfxGetMainWnd()->GetSafeHwnd() : NULL,
                      sMessage.c_str(), L"FxFile 파일·폴더 잠금",
                      MB_OK | MB_ICONWARNING | MB_TASKMODAL);
    }
    else
    {
        DWORD sAdaptiveError = ERROR_SUCCESS;
        const AdaptiveFileOperation::Result sAdaptive =
            AdaptiveFileOperation::tryExecute(&sShFileOpStruct,
                                              &sAdaptiveError);
        if (sAdaptive == AdaptiveFileOperation::ResultNotApplicable)
        {
            HRESULT sModernError = S_OK;
            const ModernShellFileOperation::Result sModern =
                ModernShellFileOperation::tryExecute(&sShFileOpStruct,
                                                     &sModernError);
            if (sModern == ModernShellFileOperation::ResultNotApplicable)
                sSucceeded = ::SHFileOperation(&sShFileOpStruct) == 0 &&
                             !sShFileOpStruct.fAnyOperationsAborted;
            else
                sSucceeded = sModern ==
                    ModernShellFileOperation::ResultSucceeded;
        }
        else
            sSucceeded = sAdaptive ==
                AdaptiveFileOperation::ResultSucceeded;
    }
