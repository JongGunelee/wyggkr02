...
xpr_bool_t SyncDirs::scan(void)
{
    ...
    if (XPR_IS_NULL(mStopEvent) || XPR_IS_NULL(mStoppedEvent))
    {
...
void SyncDirs::stop(DWORD aStopMillseconds)
{
    if (XPR_IS_NOT_NULL(mThread))
    {
        ...
        {
            // Forced termination can leave the sync map, heap, and file
            // handles corrupted.  Cancel blocking filesystem I/O and allow
            // the existing cooperative stop checks to unwind normally.
