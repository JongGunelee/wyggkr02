...
{
    // Task 065: Initialize Windows Shell system icon cache
    static xpr_bool_t sFileIconInitialized = XPR_FALSE;
    if (XPR_IS_FALSE(sFileIconInitialized))
    {
        HMODULE sShellDll = ::GetModuleHandle(XPR_STRING_LITERAL("shell32.dll"));
        if (XPR_IS_NULL(sShellDll))
            sShellDll = ::LoadLibrary(XPR_STRING_LITERAL("shell32.dll"));
        if (XPR_IS_NOT_NULL(sShellDll))
        {
