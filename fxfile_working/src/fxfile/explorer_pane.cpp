...
{
    TraceStartup(XPR_STRING_LITERAL("ExplorerPane.OnCreate.begin"));
    if (super::OnCreate(aCreateStruct) == -1)
        return -1;

    setContentsStyle(gOpt->mConfig.mShowInfoBar, gOpt->mConfig.mContentsStyle);
    TraceStartup(XPR_STRING_LITERAL("ExplorerPane.contents_style"));
    visiblePathBar(gOpt->mConfig.mShowPathBar, XPR_TRUE);
    TraceStartup(XPR_STRING_LITERAL("ExplorerPane.path_bar"));
    visibleDrivePathBar(gOpt->mConfig.mShowEachDriveBar, XPR_TRUE);
    TraceStartup(XPR_STRING_LITERAL("ExplorerPane.drive_bar"));
    visibleAddressBar(gOpt->mConfig.mShowAddressBar, XPR_TRUE);
    TraceStartup(XPR_STRING_LITERAL("ExplorerPane.address_bar"));
    visibleStatusBar(gOpt->mConfig.mShowStatusBar, XPR_TRUE);
    TraceStartup(XPR_STRING_LITERAL("ExplorerPane.status_bar"));
    ...
    TraceStartup(XPR_STRING_LITERAL("ExplorerPane.OnCreate.end"));
    return 0;
}
...
void ExplorerPane::setExplorerOption(ExplorerCtrl *aExplorerCtrl, const Option &aOption)
{
    ...
    sOption.mColumnEllipsisName               = aOption.mConfig.mFileListColumnEllipsisName;
    sOption.mColumnEllipsisSize               = aOption.mConfig.mFileListColumnEllipsisSize;
    sOption.mColumnEllipsisType               = aOption.mConfig.mFileListColumnEllipsisType;
    sOption.mColumnEllipsisDate               = aOption.mConfig.mFileListColumnEllipsisDate;
    sOption.mColumnEllipsisAttr               = aOption.mConfig.mFileListColumnEllipsisAttr;
    sOption.mColumnEllipsisExt                = aOption.mConfig.mFileListColumnEllipsisExt;
    ...
    _tcscpy(sOption.mThumbnailCachePath,        aOption.mConfig.mThumbnailCachePath);
...
{
    TraceStartup(XPR_STRING_LITERAL("ExplorerPane.setCur.begin"));
    ...
    if (XPR_IS_NOT_NULL(sNewExplorerCtrl))
    {
        ...
        if (XPR_IS_NOT_NULL(sTvItemData))
        {
            ...
            {
                sAddressBar->showCurrentPath(sTvItemData->mFullPidl);
            }
            TraceStartup(XPR_STRING_LITERAL("ExplorerPane.setCur.address"));
            ...
            TraceStartup(XPR_STRING_LITERAL("ExplorerPane.setCur.path"));
            ...
            // update status bar
            updateStatusBar();
            TraceStartup(XPR_STRING_LITERAL("ExplorerPane.setCur.status"));
        }
    ...
    recalcLayout();
    TraceStartup(XPR_STRING_LITERAL("ExplorerPane.setCur.end"));
...
void ExplorerPane::visibleAddressBar(xpr_bool_t aVisible, xpr_bool_t aLoading)
{
    if (XPR_IS_TRUE(aVisible))
    {
        ...
        if (XPR_IS_NOT_NULL(sExplorerCtrl))
        {
            LPTVITEMDATA sTvItemData = sExplorerCtrl->getFolderData();
            if (XPR_IS_NOT_NULL(sTvItemData))
                mAddressBar->showCurrentPath(sTvItemData->mFullPidl);
        }
    }
    else
    {
...
LPITEMIDLIST ExplorerPane::onFailExec(AddressBar &aAddressBar, const xpr_tchar_t *aExecPath)
{
    ...
    if (sIndex > 0)
    {
        ...
        {
            return fxfile::base::Pidl::concat(sTvItemData->mFullPidl, sLvItemData->mPidl);
        }
    }
...
void ExplorerPane::onExplore(ExplorerCtrl &aExplorerCtrl, LPITEMIDLIST aFullPidl, xpr_bool_t aUpdateBuddy, const xpr_tchar_t *aCurPath)
{
    ...
    if (sExplorerCtrlId == mCurExplorerCtrlId)
    {
        ...
        if (XPR_IS_NOT_NULL(sAddressBar) && XPR_IS_NOT_NULL(sAddressBar->m_hWnd))
        {
            if (aExplorerCtrl.isInArchiveMode() && XPR_IS_NOT_NULL(aCurPath))
                sAddressBar->SetWindowText(aCurPath);
            else
                sAddressBar->showCurrentPath(aFullPidl);
        }
...
{
    if (aExplorerCtrl.isInArchiveMode())
    {
        aExplorerCtrl.OnDrop(aOleDataObject, DROPEFFECT_COPY, CPoint(0, 0));
        return 0;
    }
    ...
    if (IsPasteInetUrl(aOleDataObject, sClipFormat.mInetUrl, sClipFormat.mFileContents, sClipFormat.mFileDescriptorW) == XPR_TRUE)
    {
    ...
    if (IsPasteFileContents(aOleDataObject, sClipFormat.mFileContents, sClipFormat.mFileDescriptorA) == XPR_TRUE)
    {
    ...
    if (IsPasteFileContents(aOleDataObject, sClipFormat.mFileContents, sClipFormat.mFileDescriptorW) == XPR_TRUE)
    {
...
void ExplorerPane::OnExpSelSingleItem(void)
{
    ...
    xpr_bool_t sIsFolder = (sLvItemData->mItemType == IDT_ARCHIVE_ITEM) ? sLvItemData->mArchiveIsDir : XPR_TEST_BITS(sLvItemData->mShellAttributes, SFGAO_FOLDER);
    if (!sIsFolder)
    {
        ...
        case SIZE_UNIT_DEFAULT:
            SizeFormat::getDefSizeFormat(sFileSize, sSize, XPR_MAX_PATH);
            break;

        case SIZE_UNIT_CUSTOM:
            SizeFormat::instance().getSizeFormat(sFileSize, sSize, XPR_MAX_PATH);
            break;

        case SIZE_UNIT_NONE:
            SizeFormat::getFormatedNumber(sFileSize, sSize, XPR_MAX_PATH);
            break;

        default:
            SizeFormat::getSizeUnitFormat(sFileSize, sUnit, sSize, XPR_MAX_PATH);
            break;
        }
    }
...
void ExplorerPane::OnExpSelMultiItem(void)
{
    ...
    {
    case SIZE_UNIT_DEFAULT:
        SizeFormat::getDefSizeFormat(sSelFileSize, sSize, XPR_MAX_PATH);
        break;

    case SIZE_UNIT_CUSTOM:
        SizeFormat::instance().getSizeFormat(sSelFileSize, sSize, XPR_MAX_PATH);
        break;

    case SIZE_UNIT_NONE:
        SizeFormat::getFormatedNumber(sSelFileSize, sSize, XPR_MAX_PATH);
        break;

    default:
        SizeFormat::getSizeUnitFormat(sSelFileSize, sUnit, sSize, XPR_MAX_PATH);
        break;
    }
    ...
    if (XPR_TEST_BITS(sTvItemData->mShellAttributes, SFGAO_FILESYSTEM))
    {
        ...
        SizeFormat::getFormatedNumber(sRealSelCount,       sRealSelCountText,       XPR_COUNT_OF(sRealSelCountText      ) - 1);
...
void ExplorerPane::updateStatusBar(xpr_uint_t aId)
{
    ...
    if (XPR_TEST_BITS(sTvItemData->mShellAttributes, SFGAO_FILESYSTEM))
    {
        ...
        SizeFormat::getFormatedNumber(sCount,       sCountText,       XPR_COUNT_OF(sCountText      ) - 1);
        ...
        {
        case SIZE_UNIT_DEFAULT:
            SizeFormat::getDefSizeFormat(sTotalSize, sExplorerCtrlData->mStatusPane1, XPR_MAX_PATH);
            break;

        case SIZE_UNIT_CUSTOM:
            SizeFormat::instance().getSizeFormat(sTotalSize, sExplorerCtrlData->mStatusPane1, XPR_MAX_PATH);
            break;

        case SIZE_UNIT_NONE:
            SizeFormat::getFormatedNumber(sTotalSize, sExplorerCtrlData->mStatusPane1, XPR_MAX_PATH);
            break;

        default:
            SizeFormat::getSizeUnitFormat(sTotalSize, sUnit, sExplorerCtrlData->mStatusPane1, XPR_MAX_PATH);
            break;
        }
    }
    else
