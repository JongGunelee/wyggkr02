...
BEGIN_MESSAGE_MAP(CfgFuncRefreshDlg, super)
    ON_BN_CLICKED(IDC_CFG_REFRESH_NO_REFRESH, OnAutoRefresh)
END_MESSAGE_MAP()
...
xpr_bool_t CfgFuncRefreshDlg::OnInitDialog(void) 
{
    ...
    SetDlgItemText(IDC_CFG_REFRESH_NO_REFRESH, gApp.loadString(XPR_STRING_LITERAL("popup.cfg.body.function.bookmark.check.auto_refresh")));
...
{
    // The persisted key is kept as the backward-compatible negative
    // mNoRefresh flag, while the user-facing option is deliberately positive.
...
void CfgFuncRefreshDlg::OnAutoRefresh(void)
{
    const xpr_bool_t sAutoRefresh =
        ((CButton *)GetDlgItem(IDC_CFG_REFRESH_NO_REFRESH))->GetCheck() == BST_CHECKED;

    GetDlgItem(IDC_CFG_REFRESH_AUTO_SORT)->EnableWindow(sAutoRefresh);
}
} // namespace cfg
} // namespace fxfile
