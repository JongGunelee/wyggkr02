...
BEGIN_MESSAGE_MAP(CfgAppearanceFolderLayoutDlg, super)
    ON_BN_CLICKED(IDC_CFG_FOLDERLAYOUT_AUTO_COLUMN_WIDTH, OnAutoColumnWidth)
...
xpr_bool_t CfgAppearanceFolderLayoutDlg::OnInitDialog(void) 
{
    ...
    SetDlgItemText(IDC_CFG_FOLDERLAYOUT_ELLIPSIS_NAME,            gApp.loadString(XPR_STRING_LITERAL("popup.cfg.body.appearance.folder_layout.check.ellipsis_name")));
    SetDlgItemText(IDC_CFG_FOLDERLAYOUT_ELLIPSIS_SIZE,            gApp.loadString(XPR_STRING_LITERAL("popup.cfg.body.appearance.folder_layout.check.ellipsis_size")));
    SetDlgItemText(IDC_CFG_FOLDERLAYOUT_ELLIPSIS_TYPE,            gApp.loadString(XPR_STRING_LITERAL("popup.cfg.body.appearance.folder_layout.check.ellipsis_type")));
    SetDlgItemText(IDC_CFG_FOLDERLAYOUT_ELLIPSIS_DATE,            gApp.loadString(XPR_STRING_LITERAL("popup.cfg.body.appearance.folder_layout.check.ellipsis_date")));
    SetDlgItemText(IDC_CFG_FOLDERLAYOUT_ELLIPSIS_ATTR,            gApp.loadString(XPR_STRING_LITERAL("popup.cfg.body.appearance.folder_layout.check.ellipsis_attr")));
    SetDlgItemText(IDC_CFG_FOLDERLAYOUT_ELLIPSIS_EXT,             gApp.loadString(XPR_STRING_LITERAL("popup.cfg.body.appearance.folder_layout.check.ellipsis_ext")));
...
void CfgAppearanceFolderLayoutDlg::onInit(const Option::Config &aConfig)
{
    ...
    ((CButton   *)GetDlgItem(IDC_CFG_FOLDERLAYOUT_ELLIPSIS_NAME     ))->SetCheck(aConfig.mFileListColumnEllipsisName);
    ((CButton   *)GetDlgItem(IDC_CFG_FOLDERLAYOUT_ELLIPSIS_SIZE     ))->SetCheck(aConfig.mFileListColumnEllipsisSize);
    ((CButton   *)GetDlgItem(IDC_CFG_FOLDERLAYOUT_ELLIPSIS_TYPE     ))->SetCheck(aConfig.mFileListColumnEllipsisType);
    ((CButton   *)GetDlgItem(IDC_CFG_FOLDERLAYOUT_ELLIPSIS_DATE     ))->SetCheck(aConfig.mFileListColumnEllipsisDate);
    ((CButton   *)GetDlgItem(IDC_CFG_FOLDERLAYOUT_ELLIPSIS_ATTR     ))->SetCheck(aConfig.mFileListColumnEllipsisAttr);
    ((CButton   *)GetDlgItem(IDC_CFG_FOLDERLAYOUT_ELLIPSIS_EXT      ))->SetCheck(aConfig.mFileListColumnEllipsisExt);
...
void CfgAppearanceFolderLayoutDlg::onApply(Option::Config &aConfig)
{
    ...
    aConfig.mFileListColumnEllipsisName     = ((CButton   *)GetDlgItem(IDC_CFG_FOLDERLAYOUT_ELLIPSIS_NAME     ))->GetCheck();
    aConfig.mFileListColumnEllipsisSize     = ((CButton   *)GetDlgItem(IDC_CFG_FOLDERLAYOUT_ELLIPSIS_SIZE     ))->GetCheck();
    aConfig.mFileListColumnEllipsisType     = ((CButton   *)GetDlgItem(IDC_CFG_FOLDERLAYOUT_ELLIPSIS_TYPE     ))->GetCheck();
    aConfig.mFileListColumnEllipsisDate     = ((CButton   *)GetDlgItem(IDC_CFG_FOLDERLAYOUT_ELLIPSIS_DATE     ))->GetCheck();
    aConfig.mFileListColumnEllipsisAttr     = ((CButton   *)GetDlgItem(IDC_CFG_FOLDERLAYOUT_ELLIPSIS_ATTR     ))->GetCheck();
    aConfig.mFileListColumnEllipsisExt      = ((CButton   *)GetDlgItem(IDC_CFG_FOLDERLAYOUT_ELLIPSIS_EXT      ))->GetCheck();
...
void CfgAppearanceFolderLayoutDlg::updateColumnEllipsisControls(void)
{
    xpr_bool_t sEnable = ((CButton *)GetDlgItem(IDC_CFG_FOLDERLAYOUT_AUTO_COLUMN_WIDTH))->GetCheck();

    const xpr_uint_t sControlIds[] =
    {
        IDC_CFG_FOLDERLAYOUT_ELLIPSIS_NAME,
        IDC_CFG_FOLDERLAYOUT_ELLIPSIS_SIZE,
        IDC_CFG_FOLDERLAYOUT_ELLIPSIS_TYPE,
        IDC_CFG_FOLDERLAYOUT_ELLIPSIS_DATE,
        IDC_CFG_FOLDERLAYOUT_ELLIPSIS_ATTR,
        IDC_CFG_FOLDERLAYOUT_ELLIPSIS_EXT,
    };

    xpr_size_t i;
    for (i = 0; i < sizeof(sControlIds) / sizeof(sControlIds[0]); ++i)
    {
        CWnd *sControl = GetDlgItem(sControlIds[i]);
        if (XPR_IS_NOT_NULL(sControl))
            sControl->EnableWindow(sEnable);
    }
}

void CfgAppearanceFolderLayoutDlg::OnAutoColumnWidth(void)
{
    updateColumnEllipsisControls();
}
