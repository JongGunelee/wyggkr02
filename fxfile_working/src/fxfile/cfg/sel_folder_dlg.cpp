...
xpr_bool_t SelFolderDlg::OnInitDialog(void) 
{
    ...
    SetWindowText(gApp.loadString(XPR_STRING_LITERAL("popup.select_folder.title")));
    SetDlgItemText(IDC_SEL_FOLDER_DESC, gApp.loadString(XPR_STRING_LITERAL("popup.select_folder.label.desc")));
...
void SelFolderDlg::OnOK(void) 
{
    ...
    if (mComboBox.GetWindowText(sFolder, XPR_MAX_PATH) <= 0)
    {
        ...
        _stprintf(sMsg, gApp.loadFormatString(XPR_STRING_LITERAL("popup.select_folder.msg.wrong_name"), XPR_STRING_LITERAL("%d")), XPR_MAX_PATH);
...
void SelFolderDlg::OnBrowse(void)
{
    ...
    LPITEMIDLIST sOldFullPidl = fxfile::Path2Pidl(sFolder);

    xpr_tchar_t sOldPath[XPR_MAX_PATH + 1] = {0};
    fxfile::GetName(sOldFullPidl, SHGDN_FORPARSING, sOldPath);

    BROWSEINFO sBrowseInfo = {0};
    sBrowseInfo.hwndOwner = GetSafeHwnd();
    sBrowseInfo.ulFlags   = 0;//BIF_USENEWUI;
    sBrowseInfo.lpszTitle = gApp.loadString(XPR_STRING_LITERAL("popup.select_folder.folder_browse.title"));
    ...
    if (sFullPidl != XPR_NULL)
    {
        ...
        fxfile::GetName(sFullPidl, SHGDN_FORPARSING, sNewPath);
