...
xpr_bool_t SearchDlg::OnInitDialog(void) 
{
    ...
    if (_tcslen(sDriveStrings) > 0)
    {
        ...
        for (i = 0; ; ++i)
        {
            ...
            fxfile::base::Pidl::create(sDrive, sFullPidl);
            if (sFullPidl != XPR_NULL)
                GetName(sFullPidl, SHGDN_INFOLDER, sName);
    ...
    SearchUserLocDeque sUserLocDeque;
    ...
    mDlgState = DlgStateManager::instance().getDlgState(XPR_STRING_LITERAL("Search"));
    ...
    case SearchLocTypeUserDefine:
        {
            ...
            cmd::SearchLocDlg sDlg;
            sDlg.setSearchLoc(&sUserLocDeque);
            sUserLocDeque.clear();

            if (sDlg.DoModal() == IDOK)
            {
                SearchUserLocDeque *sUpdatedUserLocDeque;
...
void SearchDlg::loadUserLoc(SearchUserLocDeque *aUserLocDeque)
{
    ...
    SearchUserLoc          *sUserLoc;
    SearchUserLocItem      *sUserLocItem;
    ...
    for (; sSectionIterator != sConfFile.endSection(); ++sSectionIterator)
    {
        sSection = *sSectionIterator;
        XPR_ASSERT(sSection != XPR_NULL);

        sUserLoc = new SearchUserLoc;
        sUserLoc->mName = sConfFile.getSectionName(sSection);

        sDirCount = sConfFile.getValueI(sSection, kDirCountKey, 0);
        ...
        for (i = 0; i < sDirCount; ++i)
        {
            ...
            sDir = sConfFile.getValueS(sSection, sKey, XPR_NULL);
            if (sDir == XPR_NULL)
                continue;

            sUserLocItem = new SearchUserLocItem;
            ...
            sUserLocItem->mInclude = sConfFile.getValueB(sSection, sKey, XPR_TRUE);

            // sub folder
            _stprintf(sKey, kSubFolderKey, i + 1);
            sUserLocItem->mSubFolder = sConfFile.getValueB(sSection, sKey, XPR_FALSE);
...
void SearchDlg::saveUserLoc(SearchUserLocDeque *aUserLocDeque)
{
    if (aUserLocDeque == XPR_NULL)
        return;
    ...
    fxfile::base::ConfFileEx sConfFile(sPath);
    sConfFile.setComment(XPR_STRING_LITERAL("fxfile search directory file"));
    ...
    SearchUserLoc          *sUserLoc;
    SearchUserLocItem      *sUserLocItem;
    SearchUserLocDeque::iterator sIterator;
    UserLocDeque::iterator sLocIterator;
    ...
    for (; sIterator != aUserLocDeque->end(); ++sIterator)
    {
        ...
        sSection = sConfFile.addSection(sUserLoc->mName.c_str());
        XPR_ASSERT(sSection != XPR_NULL);

        sConfFile.setValueI(sSection, kDirCountKey, (xpr_sint_t)sUserLoc->mUserLocDeque.size());
        ...
        for (i = 0; sLocIterator != sUserLoc->mUserLocDeque.end(); ++sLocIterator, ++i)
        {
            ...
            // directory
            _stprintf(sKey, kDirKey, i + 1);
            sConfFile.setValueS(sSection, sKey, sUserLocItem->mPath);

            // include
            if (sUserLocItem->mInclude == XPR_FALSE)
            {
                _stprintf(sKey, kIncludeKey, i + 1);
                sConfFile.setValueB(sSection, sKey, sUserLocItem->mInclude);
            }
            ...
            {
                _stprintf(sKey, kSubFolderKey, i + 1);
                sConfFile.setValueB(sSection, sKey, sUserLocItem->mSubFolder);
            }
        }
    }
...
void SearchDlg::getUserLoc(SearchUserLocDeque *aUserLocDeque)
{
    if (aUserLocDeque == XPR_NULL)
        return;
...
void SearchDlg::updateUserLoc(SearchUserLocDeque *aUserLocDeque)
{
    if (aUserLocDeque == XPR_NULL)
        return;
...
static xpr_size_t ValidateTarget(std::deque<xpr::string> &aTargetDeque)
{
    ...
    while (sIterator != aTargetDeque.end())
    {
        ...
        else
        {
            ...
            if (IsExistFile(sIterator->c_str()) == XPR_FALSE)
            {
                sIterator = aTargetDeque.erase(sIterator);
                continue;
            }
...
void SearchDlg::OnStart(void) 
{
    ...
    if (sCase        == XPR_TRUE) sFlags |= SearchFile::FlagCase;
    if (sSystem      == XPR_TRUE) sFlags |= SearchFile::FlagSystem;
    if (sType == 0 || sType == 1) sFlags |= SearchFile::FlagTypeFolder;
    if (sType == 0 || sType == 2) sFlags |= SearchFile::FlagTypeFile;
    ...
    if (sSearchLoc != XPR_NULL)
    {
        ...
        case SearchLocTypeItem:
            {
                xpr_tchar_t sPath[XPR_MAX_PATH + 1] = {0};
                GetName(sSearchLoc->mFullPidl, SHGDN_FORPARSING, sPath);
                                      ...
                                      xpr_size_t   aMaxElapsedSecondTimeTextLen)
{
    ...
    {
        SizeFormat::getFormatedNumber(sCount, aItemCountText, aMaxItemCountTextLen);
    }
