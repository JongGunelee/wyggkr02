...
// Task 065: Extension icon index cache to avoid repetitive SHGetFileInfo / COM calls
...
{
    if (XPR_IS_NULL(aExt) || aExt[0] == XPR_STRING_LITERAL('\0'))
        return -1;

    wchar_t sLowerKey[64] = {0};
    wcsncpy_s(sLowerKey, (const wchar_t *)aExt, 63);
    _wcslwr_s(sLowerKey);
    std::wstring sKey(sLowerKey);

    {
        xpr::MutexGuard sGuard(sExtIconMutex);
        std::map<std::wstring, xpr_sint_t>::const_iterator sIt = sExtIconCache.find(sKey);
        if (sIt != sExtIconCache.end())
            return sIt->second;
    }

    // Use a synthetic filename with extension to query system icon without disk I/O
    xpr_tchar_t sDummyName[XPR_MAX_PATH + 1] = {0};
    _tcscpy(sDummyName, XPR_STRING_LITERAL("dummy"));
    if (aExt[0] != XPR_STRING_LITERAL('.'))
        _tcscat(sDummyName, XPR_STRING_LITERAL("."));
    _tcscat(sDummyName, aExt);
    ...
    // If querying specific extension failed, fallback to generic file icon
    if (sIconIndex < 0)
    {
        if (::SHGetFileInfo(
...
static HRESULT SH_GetIDListFromObject(IUnknown *aUnknown, LPITEMIDLIST *aFullPidl)
{
    HRESULT sComResult = E_FAIL;
    ...
    if (SUCCEEDED(sComResult))
    {
        ...
        if (SUCCEEDED(sComResult))
        {
            ...
            if (SUCCEEDED(sComResult))
            {
                ...
                {
                    LPITEMIDLIST sFullPidl = XPR_NULL;
                    sComResult = SH_GetIDListFromObject(sDefaultSaveFolder, &sFullPidl);
...
void GetDriveStrings(xpr_tchar_t *aDriveStrings, xpr_sint_t aMaxLen)
{
    ...
    // The drive bar only consumes root paths ("C:\\", "D:\\", ...).
    // Enumerating the My Computer shell folder to obtain the same strings
    // invoked shell extensions and cost roughly one second per ExplorerPane.
    // The Win32 API returns the canonical multi-string directly and also
    // preserves removable/network drive roots without touching their media.
    DWORD sLength = ::GetLogicalDriveStrings((DWORD)aMaxLen, aDriveStrings);
    if (sLength == 0 || sLength >= (DWORD)aMaxLen)
        aDriveStrings[0] = XPR_STRING_LITERAL('\0');
