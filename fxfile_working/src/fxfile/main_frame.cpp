...
xpr_bool_t MainFrame::PreCreateWindow(CREATESTRUCT &aCreateStruct)
{
    ...
    if (XPR_IS_FALSE(sDefWindow))
    {
        ...
        // A snapped window can extend a few pixels beyond the work area due
        // to the invisible resize border.  Corner-only checks misclassify
        // that valid rectangle as completely off-screen; test intersection
        // instead.
        CRect sVisibleRect;
        if (::IntersectRect(&sVisibleRect, &sWorkAreaRect, &sWindowRect) == XPR_FALSE)
        {
...
{
    TraceStartup(XPR_STRING_LITERAL("MainFrame.init.begin"));
    gFrame = this;
    ...
    TraceStartup(XPR_STRING_LITERAL("MainFrame.system_image_lists"));
    ...
    TraceStartup(XPR_STRING_LITERAL("MainFrame.file_scrap"));

    // load folder layout
    FolderLayoutManager &sFolderLayoutManager = SingletonManager::get<FolderLayoutManager>();
    sFolderLayoutManager.load();
    TraceStartup(XPR_STRING_LITERAL("MainFrame.folder_layout"));

    // set option
    setOption(*gOpt);
    TraceStartup(XPR_STRING_LITERAL("MainFrame.options_applied"));

    ClipFormat::instance().registerClipFormat();
    TraceStartup(XPR_STRING_LITERAL("MainFrame.init.end"));
}

xpr_sint_t MainFrame::OnCreate(LPCREATESTRUCT aCreateStruct)
{
    TraceStartup(XPR_STRING_LITERAL("MainFrame.OnCreate.begin"));
    init();

    // create re-bar
    CreateRebar();
    TraceStartup(XPR_STRING_LITERAL("MainFrame.rebar_created"));
    ...
    m_wndReBar.mMainToolBar.showClock(gOpt->mMain.mShowClock);
    ...
    if (super::OnCreate(aCreateStruct) == -1)
        return -1;
    TraceStartup(XPR_STRING_LITERAL("MainFrame.base_OnCreate"));
...
xpr_bool_t MainFrame::OnCreateClient(LPCREATESTRUCT aCreateStruct, CCreateContext *aCreateContext)
{
    ...
    // A split lock is a separate snapshot. The ordinary split values remain
    // free to change while FxFile is running, and are used again immediately
    // after the user unlocks the layout.
    if (XPR_IS_TRUE(gOpt->mMain.mViewSplitLocked) &&
        XPR_IS_RANGE(1, gOpt->mMain.mLockedViewSplitRowCount, MAX_VIEW_SPLIT_ROW) &&
        XPR_IS_RANGE(1, gOpt->mMain.mLockedViewSplitColumnCount, MAX_VIEW_SPLIT_COLUMN))
    {
        sRowCount    = gOpt->mMain.mLockedViewSplitRowCount;
        sColumnCount = gOpt->mMain.mLockedViewSplitColumnCount;

        for (xpr_sint_t i = 0; i < XPR_COUNT_OF(gOpt->mMain.mViewSplitRatio); ++i)
        {
            gOpt->mMain.mViewSplitRatio[i] = gOpt->mMain.mLockedViewSplitRatio[i];
            gOpt->mMain.mViewSplitSize[i]  = gOpt->mMain.mLockedViewSplitSize[i];
        }
    }
    ...
    TraceStartup(XPR_STRING_LITERAL("MainFrame.OnCreateClient.views_created"));

    // Keep every ExplorerView hidden while its saved folder and history are
    // restored. One frame-owned posted message prepares all panes and then
    // publishes them together, so DWM cannot compose a partial 1->2->3->4
    // layout. WinApp can still display the top-level 2x2 skeleton first.
    mSplitter.showPane(XPR_FALSE);
    mStartupViewBatchPending = XPR_TRUE;
    mDeferStartupViews = XPR_FALSE;
...
LRESULT MainFrame::OnDeferredStartupViews(WPARAM wParam, LPARAM lParam)
{
    if (XPR_IS_FALSE(mStartupViewBatchPending))
        return 0;
    ...
    // redraw commits the completely prepared 2x2 surface to the screen.
...
void MainFrame::completeDeferredStartupViews(void)
{
    if (XPR_IS_TRUE(mStartupViewBatchPending))
        OnDeferredStartupViews(0, 0);
}
...
xpr_bool_t MainFrame::confirmToClose(xpr_bool_t aForce)
{
    if (XPR_IS_FALSE(aForce))
    {
        ...
        if (sRefCount > 0)
        {
            ...
            _stprintf(sMsg, gApp.loadFormatString(XPR_STRING_LITERAL("main_frame.msg.exit_blocked_file_operation"), XPR_STRING_LITERAL("%d")), sRefCount);
            MessageBox(sMsg, XPR_NULL, MB_ICONWARNING | MB_OK);
            return XPR_FALSE;
        }
...
void MainFrame::saveWindowPlacement(void)
{
    WINDOWPLACEMENT sWindowPlacement = {0};
    sWindowPlacement.length = sizeof(WINDOWPLACEMENT);

    // Keep the last known-good placement if Windows cannot provide a valid
    // placement during shutdown. Overwriting it with the zero-initialized
    // structure makes the next launch fall back to the default rectangle.
    if (::GetWindowPlacement(m_hWnd, &sWindowPlacement) != XPR_FALSE)
    {
        CRect sSavedRect(sWindowPlacement.rcNormalPosition);
        xpr_sint_t sSavedStatus = sWindowPlacement.showCmd;

        // Windows Snap keeps rcNormalPosition as the pre-snap restore
        // rectangle even though the visible window has already moved to a
        // snapped edge.  Save the actual rectangle for every normal window
        // (including a snapped window), but retain rcNormalPosition for a
        // maximized or minimized window so its restore rectangle is kept.
        // Do not use IsZoomed() to distinguish Snap from maximize here.
        // Windows 11 can report a snapped window as zoomed while
        // WINDOWPLACEMENT.showCmd remains SW_SHOWNORMAL.  showCmd therefore
        // provides the required split: normal/Snap uses the current rectangle;
        // true maximize/minimize keeps rcNormalPosition.
        if (sWindowPlacement.showCmd == SW_SHOWNORMAL && ::IsIconic(m_hWnd) == XPR_FALSE)
        {
            CRect sCurrentRect;
            if (::GetWindowRect(m_hWnd, &sCurrentRect) != XPR_FALSE)
            {
                // WINDOWPLACEMENT uses workspace coordinates.  Preserve that
                // convention because PreCreateWindow converts the stored
                // rectangle back to screen coordinates when a taskbar is on
                // the top or left edge of the monitor.
                CRect sWorkAreaRect;
                CRect sDesktopRect;
                GetDesktopRect(sCurrentRect, sWorkAreaRect, XPR_TRUE);
                GetDesktopRect(sCurrentRect, sDesktopRect, XPR_FALSE);

                const xpr_sint_t sXOffset = sWorkAreaRect.left - sDesktopRect.left;
                const xpr_sint_t sYOffset = sWorkAreaRect.top  - sDesktopRect.top;

                sCurrentRect.OffsetRect(-sXOffset, -sYOffset);
                sSavedRect   = sCurrentRect;
                sSavedStatus = SW_SHOWNORMAL;
            }
        }

        if (sSavedRect.Width() > 0 && sSavedRect.Height() > 0)
        {
            gOpt->mMain.mWindowStatus = sSavedStatus;
            gOpt->mMain.mWindowRect   = sSavedRect;
        }
    }
}

xpr_bool_t MainFrame::isWindowPlacementLocked(void) const
{
    return gOpt->mMain.mWindowPlacementLocked;
}

void MainFrame::setWindowPlacementLocked(xpr_bool_t aLocked)
{
    if (gOpt->mMain.mWindowPlacementLocked == aLocked)
        return;

    // Lock means "remember the window exactly as it is now". Capture the
    // current rectangle before enabling the guard so the user does not need
    // to invoke a second save command.
    if (XPR_IS_TRUE(aLocked))
    {
        saveWindowPlacement();
    }

    gOpt->mMain.mWindowPlacementLocked = aLocked;
    OptionManager::instance().saveMainOption();
}

xpr_bool_t MainFrame::isViewPathLocked(void) const
{
    return gOpt->mMain.mViewPathLocked;
}

void MainFrame::setViewPathLocked(xpr_bool_t aLocked)
{
    if (gOpt->mMain.mViewPathLocked == aLocked)
        return;
    ...
    if (XPR_IS_TRUE(aLocked))
    {
        ...
        {
            gOpt->mMain.mLockedViewPath[i][0] = XPR_STRING_LITERAL('\0');
    ...
    gOpt->mMain.mViewPathLocked = aLocked;
    OptionManager::instance().saveMainOption();
}

xpr_bool_t MainFrame::isViewSplitLocked(void) const
{
    return gOpt->mMain.mViewSplitLocked;
}

void MainFrame::setViewSplitLocked(xpr_bool_t aLocked)
{
    if (gOpt->mMain.mViewSplitLocked == aLocked)
        return;

    if (XPR_IS_TRUE(aLocked))
    {
        mSplitter.getPaneCount(&gOpt->mMain.mLockedViewSplitRowCount, &gOpt->mMain.mLockedViewSplitColumnCount);
    ...
    gOpt->mMain.mViewSplitLocked = aLocked;
    OptionManager::instance().saveMainOption();
}

xpr_bool_t MainFrame::isClockLocked(void) const
{
    return gOpt->mMain.mClockLocked;
}

void MainFrame::setClockLocked(xpr_bool_t aLocked)
{
    if (gOpt->mMain.mClockLocked == aLocked)
        return;

    gOpt->mMain.mClockLocked = aLocked;
    OptionManager::instance().saveMainOption();

    m_wndReBar.mMainToolBar.Invalidate(FALSE);
}

xpr_bool_t MainFrame::isShowClock(void) const
{
    return gOpt->mMain.mShowClock;
}

void MainFrame::setShowClock(xpr_bool_t aShow)
{
    if (gOpt->mMain.mShowClock == aShow)
        return;

    gOpt->mMain.mShowClock = aShow;
    OptionManager::instance().saveMainOption();

    m_wndReBar.mMainToolBar.showClock(aShow);
}

void MainFrame::applyUIScale(void)
{
    setChangedOption(*gOpt);
    m_wndMenuBar.updateMenuBar();
    m_wndReBar.mMainToolBar.UpdateToolbarSize();
    m_wndReBar.mBookmarkToolBar.UpdateToolbarSize();
    m_wndReBar.mDriveToolBar.UpdateToolbarSize();
    m_wndReBar.mMainToolBar.updateClockLayout();
    recalcLayout();
    RedrawWindow(XPR_NULL, XPR_NULL, RDW_INVALIDATE | RDW_UPDATENOW | RDW_ERASE | RDW_ALLCHILDREN);
}
...
void MainFrame::OnPaint(void)
{
    ...
    {
        // Real ExplorerView windows remain hidden until all four are ready.
        // Paint their final geometry in the first frame so startup shows an
        // immediate 2x2 shell instead of one large blank white rectangle.
        if (XPR_IS_FALSE(mStartupLayoutPublished) && XPR_IS_NOT_NULL(gOpt))
        {
            xpr_sint_t sRowCount = 0;
            xpr_sint_t sColumnCount = 0;
            mSplitter.getPaneCount(&sRowCount, &sColumnCount);

            const xpr_sint_t sMenuHeight = ::GetSystemMetrics(SM_CYMENU);
            const xpr_sint_t sHeaderHeight = (sMenuHeight > 22) ? sMenuHeight : 22;
            const xpr_sint_t sStatusHeight = 20;
            const xpr_sint_t sOldBkMode = sDC.SetBkMode(TRANSPARENT);
            const COLORREF sOldTextColor = sDC.SetTextColor(::GetSysColor(COLOR_WINDOWTEXT));

            for (xpr_sint_t sRow = 0; sRow < sRowCount; ++sRow)
            {
                for (xpr_sint_t sColumn = 0; sColumn < sColumnCount; ++sColumn)
                {
                    CRect sPaneRect;
                    if (XPR_IS_FALSE(mSplitter.getPaneRect(sRow, sColumn, sPaneRect)))
                        continue;

                    sDC.FillSolidRect(&sPaneRect, ::GetSysColor(COLOR_WINDOW));
                    sDC.Draw3dRect(&sPaneRect,
                                   ::GetSysColor(COLOR_3DSHADOW),
                                   ::GetSysColor(COLOR_3DHILIGHT));

                    CRect sPathRect(sPaneRect);
                    sPathRect.DeflateRect(1, 1);
                    if (sPathRect.top + sHeaderHeight < sPathRect.bottom)
                        sPathRect.bottom = sPathRect.top + sHeaderHeight;
                    sDC.FillSolidRect(&sPathRect, ::GetSysColor(COLOR_BTNFACE));

                    xpr_sint_t sViewIndex = -1;
                    if (XPR_IS_TRUE(getViewIndexFromViewSplit(sRowCount, sColumnCount,
                                                             sRow, sColumn, sViewIndex)) &&
                        XPR_IS_RANGE(0, sViewIndex, MAX_VIEW_SPLIT - 1))
                    {
                        const xpr_tchar_t *sPath = XPR_NULL;
                        if (XPR_IS_TRUE(gOpt->mMain.mViewPathLocked) &&
                            gOpt->mMain.mLockedViewPath[sViewIndex][0] != XPR_STRING_LITERAL('\0'))
                        {
                            sPath = gOpt->mMain.mLockedViewPath[sViewIndex];
                        }
                        else if (XPR_IS_FALSE(gOpt->mMain.mView[sViewIndex].mTabDeque.empty()))
                        {
                            Option::Main::Tab *sTabOption =
                                gOpt->mMain.mView[sViewIndex].mTabDeque.front();
                            if (XPR_IS_NOT_NULL(sTabOption))
                                sPath = sTabOption->mPath.c_str();
                        }

                        if (XPR_IS_NOT_NULL(sPath))
                        {
                            sPathRect.DeflateRect(7, 0);
                            sDC.DrawText(sPath, -1, &sPathRect,
                                         DT_SINGLELINE | DT_VCENTER |
                                         DT_END_ELLIPSIS | DT_NOPREFIX);
                        }
                    }

                    CRect sColumnRect(sPaneRect);
                    sColumnRect.top = sPathRect.bottom + 1;
                    if (sColumnRect.top > sColumnRect.bottom)
                        sColumnRect.top = sColumnRect.bottom;
                    if (sColumnRect.top + sHeaderHeight < sColumnRect.bottom)
                        sColumnRect.bottom = sColumnRect.top + sHeaderHeight;
                    sDC.FillSolidRect(&sColumnRect, ::GetSysColor(COLOR_BTNFACE));

                    CRect sStatusRect(sPaneRect);
                    if (sStatusRect.bottom - sStatusHeight > sStatusRect.top)
                        sStatusRect.top = sStatusRect.bottom - sStatusHeight;
                    sDC.FillSolidRect(&sStatusRect, ::GetSysColor(COLOR_BTNFACE));
                }
            }

            sDC.SetTextColor(sOldTextColor);
            sDC.SetBkMode(sOldBkMode);
            ::SetProp(m_hWnd, kStartupLayoutSkeletonProperty, (HANDLE)(INT_PTR)1);
        }
...
void MainFrame::setActiveView(xpr_sint_t aRow, xpr_sint_t aColumn)
{
    ...
    if (mPictureViewer != XPR_NULL && mPictureViewer->isDocking() == XPR_TRUE && mPictureViewer->getDockTarget() == PictureViewer::DOCK_TARGET_ACTIVE)
    {
        mPictureViewer->reattachToTargetView(PictureViewer::DOCK_TARGET_ACTIVE);
    }
}
...
void MainFrame::splitView(xpr_sint_t aRowCount, xpr_sint_t aColumnCount, xpr_bool_t aLoading)
{
    ...
    if (XPR_IS_NOT_NULL(mPictureViewer) && mPictureViewer->isDocking() == XPR_TRUE)
    {
        mPictureViewer->reattachToTargetView(mPictureViewer->getDockTarget());
    }
...
xpr_bool_t MainFrame::isStartupViewDeferred(void) const
{
    return mDeferStartupViews;
}
