...
void UserEnv::getUserEnvironment(void)
{
    ...
    // GetVersionEx lies about OS version (returns 6.2 for Win 10/11).
    // RtlGetVersion from ntdll.dll returns the true version.
    typedef LONG (WINAPI *PRtlGetVersion)(OSVERSIONINFOEX*);
    ...
    if (hNtDll != NULL)
    {
        ...
        {
            if (pRtlGetVersion(&mOSVerInfo) == 0)
                sResult = XPR_TRUE;
        }
    }
