...
{
    TraceStartup(XPR_STRING_LITERAL("OptionManager.main.begin"));
    xpr_tchar_t sPath[XPR_MAX_PATH + 1] = {0};
    ConfDir::instance().getLoadPath(ConfDir::TypeMain, sPath, XPR_MAX_PATH);

    fxfile::base::ConfFileEx sConfFile(sPath);
    xpr_bool_t sResult = sConfFile.load(XPR_STRING_LITERAL("recent_file_list"));
    TraceStartup(XPR_STRING_LITERAL("OptionManager.main.file_loaded"));

    // load main options
    mOption->loadMainOption(sConfFile);
    TraceStartup(XPR_STRING_LITERAL("OptionManager.main.options_loaded"));

    // load recent file list
    RecentFileList &sRecentFileList = RecentFileList::instance();
    sRecentFileList.scheduleLoad(sPath);
    TraceStartup(XPR_STRING_LITERAL("OptionManager.main.recent_loaded"));
...
{
    TraceStartup(XPR_STRING_LITERAL("OptionManager.config.begin"));
    xpr_tchar_t sPath[XPR_MAX_PATH + 1] = {0};
    ConfDir::instance().getLoadPath(ConfDir::TypeConfig, sPath, XPR_MAX_PATH);

    fxfile::base::ConfFileEx sConfFile(sPath);
    xpr_bool_t sResult = sConfFile.load();
    TraceStartup(XPR_STRING_LITERAL("OptionManager.config.file_loaded"));
    ...
    TraceStartup(XPR_STRING_LITERAL("OptionManager.config.end"));
