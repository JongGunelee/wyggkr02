...
PathBar::PathBar(void)
    ...
    , mInArchiveBlink(XPR_FALSE)
    , mBlinkState(XPR_FALSE)
    , mBlinkTimerId(0)
    , mPos(-1), mDownPos(-1)
{
    mPath[0]      = 0;
...
void PathBar::OnTimer(UINT_PTR nIDEvent)
{
    if (nIDEvent == 9999)
    {
        if (mInArchiveBlink)
...
void PathBar::setPathText(const xpr_tchar_t *aPathText)
{
    if (XPR_IS_NULL(aPathText))
        return;

    _tcsncpy_s(mPath, aPathText, _TRUNCATE);
    SetWindowText(mPath);

    if (_tcsncmp(mPath, XPR_STRING_LITERAL("[압축 파일]"), 7) == 0)
    {
        mInArchiveBlink = XPR_TRUE;
        if (mBlinkTimerId == 0)
        {
            mBlinkTimerId = SetTimer(9999, 500, NULL);
        }
    }
    else
    {
...
void PathBar::OnPaint(void)
{
    ...
    if (mInArchiveBlink && _tcsncmp(mPath, XPR_STRING_LITERAL("[압축 파일]"), 7) == 0)
    {
        // 1. Draw bold pulsing red "[압축 파일]" prefix
        CFont *sBoldFont = sMemDC.SelectObject(&mTextBoldFont);
        COLORREF sRedColor = mBlinkState ? RGB(255, 30, 30) : RGB(170, 0, 0);
        sMemDC.SetTextColor(sRedColor);

        const xpr_tchar_t *sTag = XPR_STRING_LITERAL("[압축 파일] ");
        CRect sTagRect(sTextRect);
        sMemDC.DrawText(sTag, (int)_tcslen(sTag), sTagRect, DT_CALCRECT | sFormat);
        sMemDC.DrawText(sTag, (int)_tcslen(sTag), sTextRect, sFormat);
        sMemDC.SelectObject(sBoldFont);

        // 2. Draw remaining path in standard style
        sMemDC.SelectObject(&mTextFont);
        sMemDC.SetTextColor(::GetSysColor(COLOR_WINDOWTEXT));
        CRect sRemainRect(sTextRect);
        sRemainRect.left += sTagRect.Width();
        const xpr_tchar_t *sRemainText = mPath + _tcslen(sTag);
        sMemDC.DrawText(sRemainText, sRemainRect, sFormat);
    }
    else
    {
...
void PathBar::updateFonts(void)
{
    ...
    LOGFONT sLogFont = {0};
    Option::getScaledFont(sLogFont, gOpt->mConfig.mCustomFontText);
    ...
    if (XPR_IS_NOT_NULL(mTextBoldFont.m_hObject))
        mTextBoldFont.DeleteObject();
    sLogFont.lfUnderline = FALSE;
    sLogFont.lfWeight = FW_BOLD;
    mTextBoldFont.CreateFontIndirect(&sLogFont);
}
