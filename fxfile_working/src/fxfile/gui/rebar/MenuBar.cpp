...
void CMenuBar::SetObserver( CMenuBarObserver *pObserver )
{
    m_pObserver = pObserver;
}

bool CMenuBar::SetMenu( UINT nID )
{
    // Delete old buttons
    while ( m_nCount > 0 )
    {
    ...
    {
        // Allocate space for buttons
        UINT nItems = m_pMenu->GetMenuItemCount();
        VERIFY( SetButtons( 0, nItems + 1 ) );
        ...
        for ( UINT nIndex = 0; nIndex < nItems; nIndex++ )
        {
            UINT nID    = m_pMenu->GetMenuItemID( nIndex );
            UINT nStyle = TBBS_BUTTON | TBBS_AUTOSIZE | TBBS_DROPDOWN;
            ...
            CString strMenu;
            m_pMenu->GetMenuText( nIndex, strMenu, MF_BYPOSITION );
...
bool CMenuBar::ReloadMenu()
{
    ...
    return SetMenu( m_nResourceID );
}

CMenu *CMenuBar::GetMenu() const
{
    return m_pMenu;
}
...
void CMenuBar::UpdateMenuBar()
{
    ...
    // Measure exact text width for each menu button to prevent truncation (...)
    CClientDC dc(this);
    CFont *pOldFont = dc.SelectObject(&m_fontMenu);
    for ( int nIndex = 0; nIndex < GetCount(); nIndex++ )
    {
        UINT nID = GetItemID(nIndex);
        if ( nID != IDBUTTON_SYSMENU && (GetButtonStyle(nIndex) & TBBS_SEPARATOR) == 0 )
        {
            CString strText = GetButtonText(nIndex);
            if ( !strText.IsEmpty() )
            {
                CRect rcCalc(0, 0, 0, 0);
                dc.DrawText(strText, &rcCalc, DT_CALCRECT | DT_SINGLELINE);
                int nPadding = (int)(24 * (sScale > 0.0 ? sScale : 1.0));
                int nCalculatedWidth = rcCalc.Width() + nPadding;
                SetButtonWidth(nID, nCalculatedWidth);
            }
        }
    }
    ...
    // Calc row height
    int cyMenu    = (int)(::GetSystemMetrics( SM_CYMENU ) * (sScale > 0.0 ? sScale : 1.0));
...
void CMenuBar::EnterTrackingMode( int nItem )
{
    if ( !m_bItemTracking )
    {
        ...
        if ( GetFocus() != this )
        {
            // This is a problem. Because the focus window is changed, it can not do command over the focus window.
            // So, I modified WM_KEYDOWN and WM_SYSKEYDOWN message processing mechanism.
            // This messages pass through PreTranslateMessage function of main frame.
            ...
            for ( int nPos = 0; strText[ nPos ]; nPos++ )
            {
                if ( strText[ nPos ] == _T('&') )
                {
                    ...
                    if ( strText[ nPos ] && ( strText[ nPos ] != _T('&') ) )
                    {
                        ...
                        ::keybd_event( (BYTE)strText[ nPos ], 0, 0, 0 );
                        ::keybd_event( (BYTE)strText[ nPos ], 0, KEYEVENTF_KEYUP, 0 );
                        break;
                    }
                }
            }
...
void CMenuBar::TrackPopupMenu()
{
    ...
    for ( int nItem = m_nItem; m_bContinue; nItem = m_nItem )
    {
        ...
        m_pMenuTracking = ( nItem == 0 ) ?
            (BCMenu *)CMenu::FromHandle(::GetSystemMenu( m_hWndMDIChild, FALSE )) :
            (BCMenu *)m_pMenu->GetSubMenu( nItem - 1 );
        if ( m_pMenuTracking == 0 )
        {
            ASSERT( false );
            break;
        }
        ...
        m_pMenuTracking->TrackPopupMenuEx(
...
BOOL CMenuBar::FramePreTranslateMessage( MSG *pMsg )
{
...
// This message depends on focus window.
...
{
    if ( ::IsWindow( m_hWnd ) && ( m_pMenuTracking != 0 ) )
    {
        ...
        m_bPrimaryMenu = ( hMenu == m_pMenuTracking->GetSafeHmenu() );
...
void CMenuBar::FrameOnMenuSelect( UINT /*nItemID*/, UINT nFlags, HMENU hSysMenu )
{
    if ( ::IsWindow( m_hWnd ) && ( m_pMenuTracking != 0 ) )
    {
        // We should know if it is primary menu and whether or not selected
        // item is submenu item to handle Esc/Left/Right keys correctly
        m_bPrimaryMenu = ( hSysMenu == m_pMenuTracking->GetSafeHmenu() );
