...
void CButtonST::DrawItem(LPDRAWITEMSTRUCT lpDIS)
{
		...
		if (m_bIsFlat == TRUE)
		{
			if (m_bDrawBorder)
			{
				if (m_xpTheme.IsAppThemed())
					m_xpTheme.DrawBackground(*pDC, BP_PUSHBUTTON, PBS_PRESSED, &itemRect, NULL);
				else
	...
	else // ...else draw non pressed button
	{
		...
		if (m_bIsFlat == TRUE)
		{
			...
			{
				if (m_xpTheme.IsAppThemed())
					m_xpTheme.DrawBackground(*pDC, BP_PUSHBUTTON, PBS_HOT, &itemRect, NULL);
				else
					OnDrawBorder(pDC, &itemRect);
			}
		}
...
/*	CClientDC clDC(GetParent());
	...
	{
		GetParent()->RedrawWindow( rect1, NULL, RDW_INTERNALPAINT | RDW_NOCHILDREN | RDW_UPDATENOW );
...
void CButtonST::SetXPTheme(BOOL bXPTheme/* = TRUE*/)
{
	if (bXPTheme)
		m_xpTheme.Open(GetSafeHwnd(), L"BUTTON");
	else
...
{
/*	if (m_bIsPressed)
		pDC->Draw3dRect(pRect, RGB(0,0,0), RGB(255,255,255));
	else
		pDC->Draw3dRect(pRect, RGB(255,255,255), RGB(0,0,0));
*/
	if (m_bIsPressed)
...
void CButtonST::OnNMThemeChanged(NMHDR *pNMHDR, LRESULT *pResult)
{
