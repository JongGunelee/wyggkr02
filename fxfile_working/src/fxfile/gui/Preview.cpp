...
void CPreviewToolBar::Init()
{
    ...
    SetImageList( ( UINT )-1, IDB_TB_MAIN_COLD_SMALL, IDB_TB_MAIN_HOT_SMALL,
        ( UINT )-1, ( UINT )-1, ( UINT )-1, TBI_SMALL );
    SetButtons( sizeof( tbButtons ) / sizeof( tbButtons[ 0 ] ), tbButtons );
    SetTextType( TBT_RIGHT );
}

bool CPreviewToolBar::IsTextOptionAvailable( int nTextType ) const
{
    switch ( nTextType )
    {
        case TBT_RIGHT:
...
bool CPreviewToolBar::IsIconOptionAvailable( int nIconType ) const
{
    switch ( nIconType )
    {
        case TBI_SMALL:
...
void CPreviewViewEx::OnPreviewClose()
{
    ...
    // m_pReBar->EnableContextMenu( true ); // CSizableReBar does not have this member
