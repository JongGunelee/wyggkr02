...
CFileDialogST::CFileDialogST(BOOL bOpenFileDialog, LPCTSTR lpszDefExt, LPCTSTR lpszFileName, DWORD dwFlags, LPCTSTR lpszFilter, CWnd* pParentWnd)
{
	...
	if (lpszFileName != NULL)
		_tcsncpy(m_szFile, lpszFileName, MAX_PATH);
...
void CFileDialogST::SetInitDir(LPCTSTR lpcszInitDir)
{
	if (!lpcszInitDir)
		return;

	m_strInitDir = lpcszInitDir;

	m_ofn.lpstrInitialDir = NULL;
	if (!m_strInitDir.empty())
		m_ofn.lpstrInitialDir = m_strInitDir.c_str();
}
...
int CFileDialogST::DoModal()
{
	...
	if (xpr::getOsVer() < xpr::kOsVerWin2000)
	{
		...
		m_ofn.lStructSize -= sizeof(void *) + sizeof(DWORD) + sizeof(DWORD);
	}
