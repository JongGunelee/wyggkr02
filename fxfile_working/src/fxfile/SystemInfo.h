...
// [Win11 Optimization] Added guard to prevent conflict with Windows SDK's
// own UNICODE_STRING definition (winternl.h / ntdef.h).
...
public:
	...
	// [Win11 Optimization] Changed DWORD to SIZE_T/ULONG_PTR for 64-bit
	// compatibility. On 64-bit Windows, virtual memory counters are 64-bit
	// sized values. Using DWORD causes data corruption/misalignment.
	...
	// [Win11 Optimization] Increased buffer from 64KB to 512KB.
	// Modern Windows 11 systems with many services/processes
	// can exceed the old 64KB buffer, causing query failures.
	enum { BufferSize = 0x80000 };
...
public:
	typedef struct _SYSTEM_HANDLE
	{
		...
		WORD	HandleType;
		WORD	HandleNumber;
		// [Win11 Optimization] Changed DWORD to ULONG_PTR for 64-bit.
		// Kernel addresses are pointer-sized (8 bytes on x64).
...
protected:
	static unsigned __stdcall GetFileNameThread( PVOID /* GetFileNameThreadParam* */ );
	static void ReleaseFileNameThreadParam( GetFileNameThreadParam* );
