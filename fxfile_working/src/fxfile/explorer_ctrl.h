...
struct FileOperationReconcileItem
{
...
typedef std::vector<FileOperationReconcileItem> FileOperationReconcileItems;
...
public:
    struct Option
    {
        ...
        xpr_bool_t  mColumnEllipsisName;
        xpr_bool_t  mColumnEllipsisSize;
        xpr_bool_t  mColumnEllipsisType;
        xpr_bool_t  mColumnEllipsisDate;
        xpr_bool_t  mColumnEllipsisAttr;
        xpr_bool_t  mColumnEllipsisExt;
        ...
        xpr_tchar_t mThumbnailCachePath[XPR_MAX_PATH + 1];
    ...
    struct DefColumnInfo
    {
        ...
        const xpr_tchar_t *mNameStringId;
        xpr_sint_t         mFormat;
        xpr_sint_t         mWidth;
        xpr_bool_t         mDefault;
    };
...
public:
    ...
    // archive mode
    xpr_bool_t   isInArchiveMode(void) const { return XPR_FALSE; }
    std::wstring getCurArchivePath(void) const { return std::wstring(); }
    std::wstring getCurArchiveSubDir(void) const { return std::wstring(); }
    xpr_bool_t   exploreArchive(const std::wstring &/*aArchivePath*/, const std::wstring &/*aSubDir*/ = L"") { return XPR_FALSE; }
    ...
    void reconcileFileOperationItems(xpr_uint_t aOperation,
...
protected:
    ...
    void       getRootDriveType(LPSHELLFOLDER aShellFolder, LPITEMIDLIST aPidl, xpr_tchar_t *aDriveType, const xpr_size_t aMaxLen) const;
    ...
    static xpr_sint_t CALLBACK DefaultItemCompareProc(LPARAM, LPARAM, LPARAM);

    // drag and drop
    CImageList *createDragImageEx(LPPOINT aPoint);
    ...
    void       enumerateShcn(ShNotifyInfo *aShNotifyInfo, xpr_bool_t aHiddenAdd);
    ...
    // Task 059 Responsive Columns Cache & Allocation Helpers
    xpr_bool_t isAutoFullColumn(const Option &aOption, const ColumnId &aColumnId) const;
    xpr_bool_t isAutomaticStandardColumn(const Option &aOption, const ColumnId &aColumnId) const;
    xpr_sint_t getSavedColumnWidth(const FolderLayout &aFolderLayout, const ColumnId &aColumnId) const;
    xpr_sint_t getDefaultColumnWidth(const ColumnId &aColumnId) const;
    xpr_bool_t isReportView(void) const;
    xpr_sint_t getAutomaticColumnMinimumWidth(const ColumnId &aColumnId) const;
    void       invalidateAutomaticColumnWidths(void);
    xpr_sint_t measureAutomaticColumnTextWidth(xpr_sint_t aColumnIndex, const ColumnId &aColumnId);
    void       measureAutomaticColumnWidths(void);
    void       reflowAutomaticColumnWidths(void);
    void       scheduleAutomaticColumnReflow(void);
    void       rememberManualColumnWidth(xpr_sint_t aColumnIndex, xpr_sint_t aWidth);
    void       adjustAutomaticColumnWidths(void);
    void       restoreSavedColumnWidths(const Option &aNewOption);
    void       invalidateColumnLayoutCache(void);
    void       cacheCurrentColumnLayout(void);
    void       applyResponsiveColumnLayout(xpr_sint_t aClientWidth);
...
protected:
    ...
    static DefColumnInfo mDefColumnDefault[];
    static DefColumnInfo mDefColumnComputer[];
    static DefColumnInfo mDefColumnVirtual[];

    DefColumnInfo *mDefColumnInfo;
    xpr_sint_t     mDefColumnCount;

    // Task 059 Responsive layout cache members
    std::vector<xpr_sint_t> mAutomaticPreferredColumnWidths;
    xpr_bool_t               mInAutomaticColumnLayout;
    xpr_sint_t               mLastAutomaticLayoutClientWidth;
