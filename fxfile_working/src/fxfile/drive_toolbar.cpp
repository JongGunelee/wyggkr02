...
#include "drive_toolbar_observer.h"
...
DWORD DriveToolBar::OnDriveProc(void)
{
    ...
    for (; it != mDriveDeque.end(); ++it)
    {
        ...
        // Do not open/probe the volume here.  A disconnected network drive or
        // sleeping removable disk can block SHGetFileInfo indefinitely and
        // make the UI wait for this worker during refresh/shutdown.
        ::SHGetFileInfo(sDrive,
...
{
    TraceStartup(XPR_STRING_LITERAL("DriveToolBar.create.begin"));
    ...
    TraceStartup(XPR_STRING_LITERAL("DriveToolBar.drive_strings"));
    ...
    TraceStartup(XPR_STRING_LITERAL("DriveToolBar.buttons_added"));

    if (DriveShcn::instance().isRegisteredWatch(this) == XPR_FALSE)
    {
        ...
        DriveShcn::instance().registerWatch(this, WM_SHCN_DRIVE, sEventMask);
    }
    TraceStartup(XPR_STRING_LITERAL("DriveToolBar.shell_watch"));
    ...
    TraceStartup(XPR_STRING_LITERAL("DriveToolBar.icon_thread"));

    UpdateToolbarSize();
    TraceStartup(XPR_STRING_LITERAL("DriveToolBar.size_updated"));

    mBarCreated = XPR_TRUE;
    mInit = XPR_TRUE;
    TraceStartup(XPR_STRING_LITERAL("DriveToolBar.create.end"));
}
...
void DriveToolBar::addDriveButton(const xpr_tchar_t *aDrive, xpr_sint_t aInsert)
{
    ...
    {
        // Parsing a disconnected drive into a PIDL may synchronously contact
        // the provider on the UI thread.  The stable drive designator is both
        // deterministic and immediately available.
        _stprintf(sText, XPR_STRING_LITERAL("%c:"), sDriveChar);
        _tcsupr(sText);
    }
...
xpr_bool_t DriveToolBar::OnNotify(WPARAM wParam, LPARAM lParam, LRESULT* pResult)
{
    if (((LPNMHDR)lParam)->code == TTN_GETDISPINFO)
    {
        ...
        // Use the already-rendered label; never bind a network provider while
        // Windows is synchronously requesting tooltip text.
        xpr::string sName = sText;
        ...
        const UINT sDriveType = ::GetDriveType(sDrive);
        if (sDriveChar != 'A' && sDriveChar != 'B' &&
            sDriveType != DRIVE_REMOTE && sDriveType != DRIVE_UNKNOWN &&
            sDriveType != DRIVE_NO_ROOT_DIR &&
            ::GetDiskFreeSpaceEx(sDrive, (PULARGE_INTEGER)&sTemp1,
                                 (PULARGE_INTEGER)&sTemp2,
                                 (PULARGE_INTEGER)&sFreeSize))
        {
...
void DriveToolBar::OnShcnUpdateDrive(Shcn *aShcn)
{
    refresh();
}

void DriveToolBar::OnShcnDriveAdd(Shcn *aShcn)
{
    refresh();
}

void DriveToolBar::OnShcnDriveRemove(Shcn *aShcn)
{
    refresh();
}

void DriveToolBar::OnShcnAllDriveUpdate(Shcn *aShcn)
{
    refresh();
}
...
void DriveToolBar::OnDropShortcut(STGMEDIUM *pstg, xpr_sint_t aDriveIndex)
{
    ...
    for (i = 0; i < sCount; ++i)
    {
        ...
        if (SetNewPath(sLinkPath, sDir, sName, XPR_STRING_LITERAL(".lnk")))
        {
            CreateShortcut(sLinkPath.c_str(), sFullPidl2);
        }
