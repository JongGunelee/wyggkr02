...
void BookmarkMgr::clear(void)
{
    if (mShellIcon != XPR_NULL)
    {
        ...
        // posted to BookmarkMgr before deleting or reloading bookmark items.
        MSG sMessage = {0};
...
LRESULT BookmarkMgr::OnShellAsyncIcon(WPARAM wParam, LPARAM lParam)
{
    ...
    if (sBookmarkIndex >= 0 && sBookmark != XPR_NULL)
    {
        ...
        DWORD_PTR sIgnored = 0;
        ::SendMessageTimeout(sHwnd, sMsg,
                             (WPARAM)sBookmarkIndex, (LPARAM)sBookmark,
                             SMTO_ABORTIFHUNG | SMTO_BLOCK, 2000, &sIgnored);
        }
    }
