...
namespace
{
bool rejectLockedPath(HWND aOwner, const xpr_tchar_t *aPath)
{
    if (aPath == XPR_NULL ||
        !FileOperationLockStore::instance().affectsLockedPath(aPath))
        return false;
    xpr::string sMessage = XPR_STRING_LITERAL("FxFile 작업 잠금으로 보호된 항목입니다.\n\n");
    sMessage += aPath;
    sMessage += XPR_STRING_LITERAL("\n\n편집 > 파일·폴더 잠금 관리에서 잠금을 해제한 뒤 다시 시도하십시오.");
    ::MessageBox(aOwner, sMessage.c_str(),
                 XPR_STRING_LITERAL("FxFile 파일·폴더 잠금"),
...
void FileDeleteCommand::execute(CommandContext &aContext)
{
    ...
    FXFILE_COMMAND_IF_FOLDER_CTRL
    {
        ...
        {
            xpr_tchar_t sSelectedPath[XPR_MAX_PATH + 1] = {0};
            GetName(sTvItemData->mShellFolder, sTvItemData->mPidl,
                    SHGDN_FORPARSING, sSelectedPath);
            if (rejectLockedPath(sFolderCtrl->GetSafeHwnd(), sSelectedPath))
                return;
    ...
    FXFILE_COMMAND_ELSE_IF_EXPLORER_CTRL
    {
        ...
        if (XPR_IS_TRUE(sTrash) || gOpt->mConfig.mExternalDeleteFileOp == XPR_TRUE)
        {
            ...
            for (xpr_sint_t i = 0;
                 i < sCount && XPR_IS_NOT_NULL(sShellFolder); ++i)
            {
                xpr_tchar_t sPath[XPR_MAX_PATH + 1] = {0};
                GetName(sShellFolder, sPidls[i], SHGDN_FORPARSING, sPath);
                if (rejectLockedPath(sExplorerCtrl->GetSafeHwnd(), sPath))
                {
...
void FolderCopyCommand::execute(CommandContext &aContext)
{
    ...
    FXFILE_COMMAND_ELSE_IF_SEARCH_RESULT_CTRL
    {
        ...
        if (XPR_IS_NOT_NULL(sFullPidl))
        {
            ...
            xpr_tchar_t *sHeapTarget = new xpr_tchar_t[XPR_MAX_PATH + 1];
            _tcscpy(sHeapTarget, sTarget);
            sHeapTarget[_tcslen(sHeapTarget) + 1] = XPR_STRING_LITERAL('\0');
            FileOpThread *sFileOpThread = new FileOpThread;
            sFileOpThread->start(XPR_TRUE, sSource, sRCount, sHeapTarget);
            sSource = XPR_NULL;
        }
...
void FolderMoveCommand::execute(CommandContext &aContext)
{
    ...
    FXFILE_COMMAND_ELSE_IF_SEARCH_RESULT_CTRL
    {
        ...
        if (XPR_IS_NOT_NULL(sFullPidl))
        {
            ...
            sHeapTarget[_tcslen(sHeapTarget) + 1] = XPR_STRING_LITERAL('\0');
            FileOpThread *sFileOpThread = new FileOpThread;
            sFileOpThread->start(XPR_FALSE, sSource, sRCount, sHeapTarget);
            sSource = XPR_NULL;
        }
