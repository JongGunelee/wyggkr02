...
xpr_sint_t HistoryDlg::addHistory(const xpr_tchar_t *aTitleText, const xpr_tchar_t *aTabText, const HistoryDeque *aHistoryDeque)
{
...
void HistoryDlg::OnTvnGetInfoTipTree(NMHDR *pNMHDR, LRESULT *pResult)
{
    ...
    *pResult = 0;
    ...
    if (sReverseIterator != sHistoryDisp->mHistoryDeque->rend())
    {
        ...
        {
            if (IsFileSystem(sFullPidl))
                GetName(sFullPidl, SHGDN_FORPARSING, pGetInfoTip->pszText);
            else
                GetFullPath(sFullPidl, pGetInfoTip->pszText);
        }
    }
}
