...
xpr_bool_t BatchRenameTabReplaceDlg::OnInitDialog(void) 
{
    ...
    SetDlgItemInt(IDC_BATCH_RENAME_REPLACE_REPEAT, FXFILE_BATCH_RENAME_REPLACE_REPEAT_DEF, XPR_FALSE);
    ...
    // Older versions did not initialize this edit control, so an untouched
    // dialog persisted Repeat=0 and every replacement became a silent no-op.
    // Migrate that legacy state to the documented one-replacement default.
    xpr_sint_t sRepeat = GetDlgItemInt(IDC_BATCH_RENAME_REPLACE_REPEAT);
