...
xpr_sint_t FileIntegrityMonitorCommand::canExecute(CommandContext &aContext)
{
    FXFILE_COMMAND_DECLARE_CTRL;

    xpr_bool_t sEnable = XPR_FALSE;
    if (XPR_IS_NOT_NULL(sExplorerCtrl) && XPR_IS_TRUE(sExplorerCtrl->isFileSystemFolder()))
        sEnable = (sExplorerCtrl->GetSelectedCount() > 0) ? XPR_TRUE : XPR_FALSE;

    return XPR_IS_TRUE(sEnable) ? StateEnable : StateDisable;
}

void FileIntegrityMonitorCommand::execute(CommandContext &aContext)
{
    FXFILE_COMMAND_DECLARE_CTRL;

    if (XPR_IS_NULL(sExplorerCtrl) || sExplorerCtrl->GetSelectedCount() <= 0)
        return;

    FileSysItemDeque sFileSysItemDeque;
    sExplorerCtrl->getSelFileSysItems(sFileSysItemDeque, XPR_FALSE, SFGAO_FILESYSTEM, SFGAO_FOLDER);
    if (sFileSysItemDeque.empty())
        return;

    FileIntegrityDlg sDlg;
    for (FileSysItemDeque::iterator sIterator = sFileSysItemDeque.begin(); sIterator != sFileSysItemDeque.end(); ++sIterator)
    {
        FileSysItem *sFileSysItem = *sIterator;
        if (XPR_IS_NOT_NULL(sFileSysItem))
            sDlg.addPath(sFileSysItem->mPath.c_str());
    }
