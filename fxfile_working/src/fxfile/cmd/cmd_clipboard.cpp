...
static bool TryRestoreArchiveClipboardFromOS(void)
{
    if (ArchiveClipboard::instance().hasData())
        return true;
    ...
    std::wstring sContent = pText;
    ::GlobalUnlock(hData);
    ::CloseClipboard();

    const std::wstring sHeader = L"__FXFILE_ARCHIVE_COPY__";
    if (sContent.rfind(sHeader, 0) != 0)
        return false;

    std::vector<std::wstring> sLines;
    std::wistringstream sStream(sContent);
    std::wstring sLine;
    while (std::getline(sStream, sLine))
    {
        if (!sLine.empty() && sLine.back() == L'\r')
            sLine.pop_back();
        sLines.push_back(sLine);
    }

    if (sLines.size() < 4)
        return false; // Header, ArchivePath, SubDir, at least 1 item

    std::wstring sArchivePath = sLines[1];
    std::wstring sSubDir = sLines[2];
    std::vector<std::wstring> sItems;
    for (size_t i = 3; i < sLines.size(); ++i)
    {
        if (!sLines[i].empty())
            sItems.push_back(sLines[i]);
    }

    if (sItems.empty())
        return false;

    ArchiveClipboard::instance().setSelection(sArchivePath, sSubDir, sItems);
    return true;
}

static xpr_bool_t IsFilePaste(ExplorerCtrl &aExplorerCtrl, MainFrame &aMainFrame)
{
    if (TryRestoreArchiveClipboardFromOS())
        return XPR_TRUE;
    ...
    if (aExplorerCtrl.isInArchiveMode())
    {
        ...
        sOleDataObject.AttachClipboard();
        ClipFormat &sClipFormat = ClipFormat::instance();
        if (sOleDataObject.IsDataAvailable(CF_HDROP) || sOleDataObject.IsDataAvailable(sClipFormat.mShellIDList))
            return XPR_TRUE;
        return XPR_FALSE;
    }
...
void ClipboardFilePasteCommand::execute(CommandContext &aContext)
{
    ...
    FXFILE_COMMAND_IF_FOLDER_CTRL
    {
        ...
        const bool sFxLocksActive = FileOperationLockStore::instance().hasLocks();
        xpr_bool_t sExternalCopyFileOp = (sDropEffect == DROPEFFECT_COPY) && XPR_IS_TRUE(gOpt->mConfig.mExternalCopyFileOp) && !sFxLocksActive;
        xpr_bool_t sExternalMoveFileOp = (sDropEffect == DROPEFFECT_MOVE) && XPR_IS_TRUE(gOpt->mConfig.mExternalMoveFileOp) && !sFxLocksActive;
    ...
    FXFILE_COMMAND_ELSE_IF_EXPLORER_CTRL
    {
        ...
        else if (TryRestoreArchiveClipboardFromOS() && !sExplorerCtrl->isInArchiveMode())
        {
            const std::wstring &sArchivePath = ArchiveClipboard::instance().getArchivePath();
            const std::vector<std::wstring> &sItems = ArchiveClipboard::instance().getItemNames();
            
            xpr_tchar_t sTargetDirBuf[XPR_MAX_PATH + 1] = {0};
            const xpr_tchar_t *sCur = sExplorerCtrl->getCurPath();
            if (sCur != XPR_NULL && _tcslen(sCur) > 0)
                _tcscpy_s(sTargetDirBuf, sCur);
            else
                _tcscpy_s(sTargetDirBuf, sExplorerCtrl->getCurFullPath());

            if (_tcslen(sTargetDirBuf) > 0 && !sItems.empty())
            {
                archive::ArchiveProgressDialog sProgressDlg;
                sProgressDlg.create(sExplorerCtrl->GetSafeHwnd(), L"7-Zip 압축 풀기", L"압축 파일에서 항목을 추출하는 중입니다...");
                ...
                bool sOk = archive::ArchiveEngine::extractItems(sArchivePath, sItems, sTargetDirBuf, sCallback);
                sProgressDlg.close();

                if (sOk)
                {
                    ::SHChangeNotify(SHCNE_UPDATEDIR, SHCNF_PATH | SHCNF_FLUSH, (LPCVOID)sTargetDirBuf, XPR_NULL);
                    sExplorerCtrl->refresh();
                }
            }
            return;
        }
        else if (sExplorerCtrl->isInArchiveMode())
        {
            COleDataObject sOleDataObject;
            sOleDataObject.AttachClipboard();

            std::vector<std::wstring> sSrcFiles;

            if (sOleDataObject.IsDataAvailable(CF_HDROP))
            {
                STGMEDIUM sMedium;
                if (sOleDataObject.GetData(CF_HDROP, &sMedium))
                {
                    HDROP hDrop = (HDROP)::GlobalLock(sMedium.hGlobal);
                    if (hDrop != NULL)
                    {
                        UINT sCount = ::DragQueryFileW(hDrop, 0xFFFFFFFF, NULL, 0);
                        for (UINT i = 0; i < sCount; ++i)
                        {
                            wchar_t sFilePath[MAX_PATH] = {0};
                            if (::DragQueryFileW(hDrop, i, sFilePath, MAX_PATH) > 0)
                            {
                                sSrcFiles.push_back(sFilePath);
                            }
                        }
            ...
            if (!sSrcFiles.empty())
            {
                std::wstring sArchivePath = sExplorerCtrl->getCurArchivePath();
                std::wstring sSubDir = sExplorerCtrl->getCurArchiveSubDir();

                archive::ArchiveProgressDialog sProgressDlg;
                sProgressDlg.create(sExplorerCtrl->GetSafeHwnd(), L"7-Zip 압축 추가", L"압축 파일에 항목을 추가하는 중입니다...");
                auto sCallback = [&sProgressDlg](int aPercent, const std::wstring &aFile) -> bool {
                    if (sProgressDlg.isCancelled())
                        return false;
                    sProgressDlg.update(aPercent, aFile);
                ...
                bool sOk = archive::ArchiveEngine::addItemsToArchive(sArchivePath, sSubDir, sSrcFiles, sCallback);
                sProgressDlg.close();

                if (sOk)
                {
                    sExplorerCtrl->exploreArchive(sArchivePath, sSubDir);
                }
            }
            return;
        }
