...
xpr_bool_t BatchRename::rename(const FileNameFormat &aFileNameFormat, xpr_bool_t aAddHistory)
{
    ...
    {
        // A new user operation starts a new history branch. Redo entries
        // from a previously undone branch must never survive this point.
        clearForward();

        if (XPR_IS_TRUE(isFlag(FlagHistoryArchive)))
            addHistory(aFileNameFormat);
    }

    return XPR_IS_FALSE(sAtLeastOneError);
}
...
void BatchRename::syncRenamedItems(void)
{
    if (XPR_IS_NULL(mMultiRename))
        return;

    xpr_size_t sIndex = 0;
    ItemDeque::iterator sIterator = mItemDeque.begin();
    for (; sIterator != mItemDeque.end(); ++sIterator, ++sIndex)
    {
        Item *sItem = *sIterator;
        if (XPR_IS_NULL(sItem))
            continue;

        xpr::string sActualOldName;
        if (XPR_IS_TRUE(mMultiRename->getItemOldName(sIndex, sActualOldName)))
            sItem->mOld = sActualOldName;
    }
}
