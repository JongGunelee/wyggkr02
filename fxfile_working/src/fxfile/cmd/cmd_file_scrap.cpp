...
void FileScrapAddCommand::execute(CommandContext &aContext)
{
    ...
    FXFILE_COMMAND_IF_FOLDER_CTRL
    {
        ...
        if (XPR_IS_NOT_NULL(sTvItemData))
        {
            ...
            sFileScrap.addItem(sGroupId, sTvItemData->mShellFolder, (LPCITEMIDLIST *)&sTvItemData->mPidl, 1);
        }
    }
    FXFILE_COMMAND_ELSE_IF_SEARCH_RESULT_CTRL
    {
...
{
    FileScrap::Group *sGroup = aFileScrapPane->getCurGroup();
...
xpr_sint_t FileScrapCopyToCommand::canExecute(CommandContext &aContext)
{
    ...
    FXFILE_COMMAND_IF_FILE_SCRAP_PANE
    {
...
void FileScrapValidateCommand::execute(CommandContext &aContext)
{
    ...
    FXFILE_COMMAND_IF_FILE_SCRAP_PANE
    {
        FileScrap::Group *sGroup = sFileScrapPane->getCurGroup();
        if (XPR_IS_NULL(sGroup))
            return;
...
void FileScrapGroupAddCommand::execute(CommandContext &aContext)
{
    ...
    FXFILE_COMMAND_IF_FILE_SCRAP_PANE
    {
        ...
        xpr_uint_t sGroupId = FileScrap::InvalidGroupId;
