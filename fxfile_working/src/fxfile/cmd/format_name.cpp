...
void appendNameRange(xpr::string &aTarget, const xpr::string &aSource,
                     xpr_sint_t aBeginPos, xpr_sint_t aEndPos)
{
    const xpr_sint_t sLength = (xpr_sint_t)aSource.length();
    xpr_sint_t sBeginPos = (aBeginPos < 0) ? 0 : aBeginPos;
    if (sBeginPos >= sLength)
        return;

    if (aEndPos < 0)
    {
        aTarget += aSource.substr(sBeginPos);
        return;
    }

    if (aEndPos <= sBeginPos)
        return;

    xpr_sint_t sCount = aEndPos - sBeginPos;
    if (sCount > sLength - sBeginPos)
        sCount = sLength - sBeginPos;

    aTarget += aSource.substr(sBeginPos, sCount);
}
} // namespace anonymous
...
void FormatBaseFileName::rename(RenameContext &aContext) const
{
    ...
    RenameContext::getBaseFileNameAndExt(aContext.mFolder, aContext.mOldFileName, sBaseFileName, sFileExt);
    appendNameRange(aContext.mNewFileName, sBaseFileName, mBeginPos, mEndPos);
}
...
void FormatFileExt::rename(RenameContext &aContext) const
{
    ...
    RenameContext::getBaseFileNameAndExt(aContext.mFolder, aContext.mOldFileName, sBaseFileName, sFileExt);
    appendNameRange(aContext.mNewFileName, sFileExt, mBeginPos, mEndPos);
}
...
{
    appendNameRange(aContext.mNewFileName, aContext.mOldFileName, mBeginPos, mEndPos);
}
...
{
    appendNameRange(aContext.mNewFileName, aContext.mOrgFileName, mBeginPos, mEndPos);
}
