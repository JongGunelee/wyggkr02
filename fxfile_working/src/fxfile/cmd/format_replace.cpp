...
void FormatReplace::rename(RenameContext &aContext) const
{
    ...
    if (sRepeat == 0)
    {
        // Legacy dialog state/XML could persist zero even though the UI means
        // "replace once". Keep old saved operations functional.
        sRepeat = 1;
    }
    else if (sRepeat < 0)
    {
    ...
    for (i = 0; i < sRepeat; ++i)
    {
        ...
        if (XPR_IS_TRUE(mCaseSensitivity))
        {
            sFind = aContext.mNewFileName.find(mFindString, sOffset);
        }
        else
        {
            xpr::string sSearchText = aContext.mNewFileName;
            xpr::string sSearchFind = mFindString;
            sSearchText.upper_case();
            sSearchFind.upper_case();
            sFind = sSearchText.find(sSearchFind, sOffset);
        }

        if (sFind == xpr::string::npos)
            break;

        aContext.mNewFileName.erase(sFind, mFindString.length());
        aContext.mNewFileName.insert(sFind, mReplaceString);
        sOffset = sFind + mReplaceString.length();
    }
}
