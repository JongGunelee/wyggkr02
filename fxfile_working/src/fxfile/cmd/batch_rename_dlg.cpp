...
xpr_bool_t BatchRenameDlg::OnInitDialog(void)
{
    ...
    if (XPR_IS_NOT_NULL(mDlgState))
    {
        ...
        // "Rename by Result" and "Batch Format Archive" were written by
        // older builds while the load path looked for different keys. Keep
        // reading those legacy names so existing portable state is migrated.
        sResultApply    = mDlgState->getStateB(
            XPR_STRING_LITERAL("Result Apply"),
            mDlgState->getStateB(XPR_STRING_LITERAL("Rename by Result"), XPR_TRUE));
        sHistoryArchive = mDlgState->getStateB(
            XPR_STRING_LITERAL("History Archive"),
            mDlgState->getStateB(XPR_STRING_LITERAL("Batch Format Archive"), XPR_TRUE));
...
void BatchRenameDlg::OnDestroy(void)
{
    ...
    if (XPR_IS_NOT_NULL(mDlgState))
    {
        ...
        mDlgState->setStateB(XPR_STRING_LITERAL("Result Apply"),          XPR_TEST_BITS(sFlags, BatchRename::FlagResultRename));
        mDlgState->setStateB(XPR_STRING_LITERAL("History Archive"),       XPR_TEST_BITS(sFlags, BatchRename::FlagHistoryArchive));
...
LRESULT BatchRenameDlg::OnFinalize(WPARAM wParam, LPARAM lParam)
{
    ...
    if (sStatus == MultiRename::StatusRenameFailed ||
        sStatus == MultiRename::StatusStopped)
    {
        // A batch can be partly committed before a later item fails or the
        // user stops it. Rebase the preview model to the physical filenames
        // so retry never targets names which no longer exist.
        mBatchRename->syncRenamedItems();
    }
    ...
    case MultiRename::StatusRenameFailed:
        {
            mListCtrl.Invalidate(XPR_FALSE);
            mListCtrl.UpdateWindow();

            const xpr_sint_t sFailedIndex = mBatchRename->getInvalidItem();
            if (sFailedIndex >= 0 && (xpr_size_t)sFailedIndex < sCount)
            {
                mListCtrl.SetFocus();
                mListCtrl.EnsureVisible(sFailedIndex, XPR_TRUE);
                mListCtrl.SetSelectionMark(sFailedIndex);
                mListCtrl.SetItemState(sFailedIndex,
                    LVIS_FOCUSED | LVIS_SELECTED,
                    LVIS_FOCUSED | LVIS_SELECTED);
            }

            const xpr_tchar_t *sMsg = gApp.loadString(
                XPR_STRING_LITERAL("popup.batch_rename.msg.rename_failed"));
            MessageBox(sMsg, XPR_NULL, MB_OK | MB_ICONSTOP);

            xpr_tchar_t sStatusText[0xff] = {0};
            _stprintf_s(sStatusText, XPR_COUNT_OF(sStatusText),
                gApp.loadFormatString(
                    XPR_STRING_LITERAL("popup.batch_rename.status.failed"),
                    XPR_STRING_LITERAL("%d,%d")),
                sRenamedCount, sCount);
            setStatus(sStatusText);
            break;
        }
...
{
    BatchRenameTabFormatDlg *sDlg = (BatchRenameTabFormatDlg *)getTabDialog(0);
...
void BatchRenameDlg::OnReplaceApply(void)
{
    ...
    if (sRepeat < FXFILE_BATCH_RENAME_REPLACE_REPEAT_MIN)
    {
        sDlg->SetDlgItemInt(IDC_BATCH_RENAME_REPLACE_REPEAT, FXFILE_BATCH_RENAME_REPLACE_REPEAT_DEF, XPR_FALSE);
        sRepeat = FXFILE_BATCH_RENAME_REPLACE_REPEAT_DEF;
    }
