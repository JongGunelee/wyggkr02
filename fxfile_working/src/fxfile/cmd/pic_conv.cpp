...
{
    mThread.stop();
    // The conversion worker can be sleeping indefinitely while it waits for
    // the colour-depth dialog.  Wake that wait before joining, otherwise a
    // dialog cancel/close deadlocks the UI thread forever.
    if (XPR_IS_NOT_NULL(mEvent))
        ::SetEvent(mEvent);
    mThread.join();
}
