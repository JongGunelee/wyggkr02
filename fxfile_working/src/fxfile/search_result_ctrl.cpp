...
#include "search_result_ctrl_observer.h"
...
xpr_sint_t SearchResultCtrl::OnCreate(LPCREATESTRUCT aCreateStruct)
{
    ...
    mShcnId = ShellChangeNotify::instance().registerWatch(this, WM_SHELL_CHANGE_NOTIFY, XPR_NULL, sEventMask, XPR_TRUE);
...
void SearchResultCtrl::OnDestroy(void)
{
    ...
    MSG sShellMessage = {0};
    while (::PeekMessage(&sShellMessage, m_hWnd,
                         WM_SHELL_CHANGE_NOTIFY,
                         WM_SHELL_CHANGE_NOTIFY, PM_REMOVE))
    {
        Shcn *sShcn = reinterpret_cast<Shcn *>(sShellMessage.wParam);
...
void SearchResultCtrl::onStartSearch(void)
{
    ...
    {
        xpr::MutexGuard aLockGuard(mMutex);
...
void SearchResultCtrl::OnGetdispinfo(NMHDR *aNmHdr, LRESULT *aResult) 
{
    ...
    *aResult = 0;
    ...
    if (sLvItem.mask & LVIF_TEXT)
    {
        ...
        case 2:
            GetFileSize(sPath, sLvItem.pszText, sLvItem.cchTextMax);
            break;

        case 3:
            GetFileType(sPath, sLvItem.pszText);
            break;

        case 4:
            GetFileTime(sPath, sLvItem.pszText);
            break;
        }
    }
...
void SearchResultCtrl::executeLink(LPSHELLFOLDER aShellFolder, LPCITEMIDLIST aPidl)
{
    xpr_tchar_t sPath[XPR_MAX_PATH + 1] = {0};
...
void SearchResultCtrl::executeUrl(LPSHELLFOLDER aShellFolder, LPCITEMIDLIST aPidl)
{
    HRESULT sHResult;
...
void SearchResultCtrl::executeLinkFolder(LPCITEMIDLIST aFullPidl)
{
    if (XPR_IS_NOT_NULL(mObserver))
    {
...
void SearchResultCtrl::doExecuteError(LPSHELLFOLDER aShellFolder, LPCITEMIDLIST aPidl)
{
    xpr_tchar_t sPath[XPR_MAX_PATH + 1];
...
void SearchResultCtrl::OnShcnRename(Shcn *aShcn)
{
    xpr_tchar_t sPath1[XPR_MAX_PATH + 1] = {0};
    xpr_tchar_t sPath2[XPR_MAX_PATH + 1] = {0};

    GetName(aShcn->mPidl1, SHGDN_FORPARSING, sPath1);
    GetName(aShcn->mPidl2, SHGDN_FORPARSING, sPath2);
...
void SearchResultCtrl::OnShcnUpdateDir(Shcn *aShcn)
{
    refresh();
}
...
void SearchResultCtrl::OnBeginlabeledit(NMHDR *aNmHdr, LRESULT *aResult)
{
    ...
    *aResult = 0;
        ...
        !XPR_TEST_BITS(sSrItemData->mFileAttributes, FILE_ATTRIBUTE_DIRECTORY))
    {
        ...
        xpr_tchar_t *sExt = (xpr_tchar_t *)GetFileExt(sEditName);
        if (XPR_IS_NOT_NULL(sExt))
        {
            ...
            if (sFindFile == INVALID_HANDLE_VALUE)
            {
                ...
                *sSplit = '\\';
                ...
                if (::MoveFile(sOldPath, sNewPath) == XPR_TRUE)
                {
                    ...
                    GetFileType(sNewPath, sTypeName);
                    ...
                    sLvItem.iSubItem = 0;
                    sLvItem.iImage   = GetItemIconIndex(sNewPath);
                    SetItem(&sLvItem);
...
xpr_sint_t CALLBACK SearchResultCtrl::DefaultItemCompareProc(LPARAM lParam1, LPARAM lParam2, LPARAM lParamSort)
{
    ...
    else if (sColumn == 3)
    {
        if ((sSrItemData1->mFileAttributes & FILE_ATTRIBUTE_DIRECTORY) == (sSrItemData2->mFileAttributes & FILE_ATTRIBUTE_DIRECTORY))
        {
            ...
            {
                xpr_tchar_t *sExt1 = (xpr_tchar_t *)GetFileExt(sSrItemData1->mFileName);
                xpr_tchar_t *sExt2 = (xpr_tchar_t *)GetFileExt(sSrItemData2->mFileName);
