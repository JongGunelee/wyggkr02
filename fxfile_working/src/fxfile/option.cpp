...
static const OptionKey gConfigOptionKeys[] =
{
    ...
    { XPR_STRING_LITERAL("config.file_list.column_ellipsis_ext"),              OptionKey::TypeBoolean, &Option::mConfig.mFileListColumnEllipsisExt,    (void *)XPR_TRUE                       },
...
{
    memset(&aOutLogFont, 0, sizeof(LOGFONT));
    if (gOpt != XPR_NULL && gOpt->mConfig.mCustomFont == XPR_TRUE && aCustomFontText != XPR_NULL && aCustomFontText[0] != _T('\0'))
    {
        StringToLogFont(aCustomFontText, aOutLogFont);
    }
    else
    {
        NONCLIENTMETRICS sNcm = {0};
        sNcm.cbSize = sizeof(NONCLIENTMETRICS);
        ::SystemParametersInfo(SPI_GETNONCLIENTMETRICS, sizeof(NONCLIENTMETRICS), &sNcm, 0);
