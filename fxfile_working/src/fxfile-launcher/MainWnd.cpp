...
void MainWnd::getSettingPath(LPTSTR aPath)
{
    ...
    TCHAR sModuleDir[MAX_PATH + 1] = {0};
    ::GetModuleDir(sModuleDir, MAX_PATH);

    TCHAR sLocalDir[MAX_PATH + 1] = {0};
    _tcscpy(sLocalDir, sModuleDir);
    _tcscat(sLocalDir, _T("\\fxfile-launcher"));

    // 1. Priority: Local folder in build directory
    if (::GetFileAttributes(sLocalDir) != INVALID_FILE_ATTRIBUTES)
    {
        _tcscpy(aPath, sLocalDir);
    }
    // 2. Fallback: System AppData
    else if (::SHGetSpecialFolderPath(NULL, aPath, CSIDL_APPDATA, FALSE))
    {
...
LRESULT MainWnd::OnKeyHooked(WPARAM wParam, LPARAM lParam)
{
    ...
    // Try stable version first, then fallback to original
    TCHAR sStablePath[MAX_PATH + 1] = {0};
    _tcscpy(sStablePath, sDir);
    _tcscat(sStablePath, _T("\\fxfile_stable.exe"));

    if (::GetFileAttributes(sStablePath) != INVALID_FILE_ATTRIBUTES)
    {
        _tcscpy(sPath, sStablePath);
    }
    else
    {
#ifdef _DEBUG
