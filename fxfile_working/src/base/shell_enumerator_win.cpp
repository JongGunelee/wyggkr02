...
xpr_bool_t ShellEnumerator::enumerate(HWND aHwnd, LPSHELLFOLDER aShellFolder, xpr_sint_t aListType, xpr_sint_t aAttributes)
{
    ...
    // Task 064c: Pass NULL instead of the UI window handle so that the Shell
    // cannot pump synchronous UI messages (auth dialogs, UAC prompts, COM
    // surrogate callbacks) back through the UI thread during enumeration.
    // Passing the real HWND caused the UI thread to process arbitrary Shell
    // COM messages while waiting for EnumObjects to return, which is what
    // produced the "Not Responding" hang on folders with many items or active
    // shell extensions (e.g., cloud-sync, AV, VS solution handlers).
