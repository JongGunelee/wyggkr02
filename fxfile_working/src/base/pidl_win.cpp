...
LPITEMIDLIST Pidl::getFullPidl(LPSHELLFOLDER aShellFolder, LPCITEMIDLIST aSimplePidl)
{
    ...
    if (SUCCEEDED(sComResult))
    {
        ...
        if (SUCCEEDED(sComResult))
        {
            ...
            // IDataObject::GetData transfers ownership of the returned
            // STGMEDIUM to the caller. Release it even when
            // pUnkForRelease is NULL; in that case hGlobal is released.
