...
{
    // xpr::XmlReader's file-path overload narrows a Unicode path before it
    // reaches libxml2.  The former workaround copied every language file to
    // %TEMP% for each parse, which doubled filesystem/antivirus work during
    // startup.  Read the original path with the native W API and let libxml2
    // parse the bytes from memory instead.  This keeps Korean paths reliable
    // without creating or rescanning temporary files.
    HANDLE sFile = ::CreateFileW((const wchar_t *)aPath,
                                 GENERIC_READ,
    ...
    if (::GetFileSizeEx(sFile, &sFileSize) == FALSE ||
        sFileSize.QuadPart <= 0 || sFileSize.QuadPart > 0x7fffffff)
    {
        ::CloseHandle(sFile);
        return XPR_FALSE;
    }

    std::vector<xpr_byte_t> sBuffer((xpr_size_t)sFileSize.QuadPart);
    DWORD sReadBytes = 0;
    xpr_bool_t sRead = (::ReadFile(sFile,
                                  ...
                                  &sReadBytes,
                                  XPR_NULL) != FALSE &&
                        sReadBytes == (DWORD)sBuffer.size());
    ::CloseHandle(sFile);
    if (XPR_IS_FALSE(sRead))
        return XPR_FALSE;

    XmlReader sXmlReader;
    if (sXmlReader.load(&sBuffer[0], sBuffer.size()) != XPR_TRUE)
        return XPR_FALSE;
    ...
    _tcscpy_s(mPath, XPR_MAX_PATH + 1, aPath);

    // [x64 Debug] Verify string count
    if (XPR_IS_NOT_NULL(aStringTable))
    {
        size_t sCount = aStringTable->mStringTable.size();
        if (sCount == 0)
        {
            xpr_tchar_t sErr[1024];
            _stprintf(sErr, XPR_STRING_LITERAL("XML Parse Error: Loaded 0 strings from %s\nPlease check file encoding (should be UTF-8)."), aPath);
            MessageBox(NULL, sErr, XPR_STRING_LITERAL("Language Debug"), MB_OK | MB_ICONERROR);
        }
    }
