...
xpr_bool_t ConfFile::load(const xpr_tchar_t *aStopSection)
{
    xpr_rcode_t sRcode;
    xpr::FileIo sFileIo;
    ...
    do
    {
        ...
        case '[':
            ...
            if (XPR_IS_NOT_NULL(sSeparator))
            {
                ...
                // Large append-only sections (for example 8,000+ recent
                // files) need not be materialized as key objects merely to
                // read the small startup layout above them.  The caller can
                // load that section through a purpose-built reader.
                if (XPR_IS_NOT_NULL(aStopSection) &&
                    _tcsicmp(sLineRef + 1, aStopSection) == 0)
                {
                    sReadLen = -1;
                    break;
                }
